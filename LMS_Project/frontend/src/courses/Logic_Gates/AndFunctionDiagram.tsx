...
const TABS: readonly { key: DiagramVariant; label: string }[] = [
  { key: "two", label: "2-input AND" },
  { key: "three", label: "3-input AND" },
  { key: "six", label: "6-input AND" },
];
...
function stepText(variant: DiagramVariant, step: SimulationStep) {
  if (variant === "two") {
    ...
    if (step === 2) return "The AND calculation area checks A · B.";
    if (step === 3) return "The output path carries the result.";
    return "Q shows the final AND output.";
  }

  if (variant === "three") {
    if (step === 1) return "Inputs A, B, and C are selected.";
    if (step === 2) return "The AND calculation area checks A · B · C.";
    if (step === 3) return "The output path carries the result.";
    return "Q shows the final AND output.";
  }
  ...
  if (step === 3) return "The output path moves toward Q.";
  return "Q shows the final AND output.";
}
...
function AndGate({ x, y, w, h, ampSize = 23, active = false }: AndGateProps) {
  const offsetX = COMPONENT_OFFSET.andGate.x;
  const offsetY = COMPONENT_OFFSET.andGate.y;

  return (
    <g>
      <path
        d={andGatePath(x + offsetX, y + offsetY, w, h)}
...
function TwoInputAndGate({
  ...
  return (
    ...
    >
      ...
      <AndGate x={66} y={1} w={79} h={57} ampSize={20} active={activeGate} />
...
function ThreeInputAndGate({
  ...
  return (
    ...
    >
      ...
      {activeA ? (
        <ActiveWire d={PATH.three.inputA} />
      ) : (
        <Wire d={PATH.three.inputA} />
      )}
      {activeB ? (
        <ActiveWire d={PATH.three.inputB} />
      ) : (
        <Wire d={PATH.three.inputB} />
      )}
      {activeC ? (
        <ActiveWire d={PATH.three.inputC} />
      ) : (
        <Wire d={PATH.three.inputC} />
      )}

      <AndGate x={72} y={2} w={75} h={58} ampSize={20} active={activeGate} />

      {activeOutput ? (
        <ActiveWire d={PATH.three.Q} />
      ) : (
        <Wire d={PATH.three.Q} />
      )}
...
function SixInputAndFunction({
  ...
  return (
    ...
    >
      ...
      <AndGate x={75} y={1} w={76} h={58} ampSize={23} active={firstGateActive.AB} />
      <AndGate x={75} y={78} w={76} h={58} ampSize={23} active={firstGateActive.CD} />
      <AndGate x={75} y={154} w={76} h={58} ampSize={23} active={firstGateActive.EF} />
      ...
      <AndGate x={245} y={93} w={84} h={58} ampSize={23} active={finalGateActive} />
...
function FormulaPanel({ variant, inputs, logic }: FormulaPanelProps) {
  const evaluated =
    variant === "two"
      ? `Q = ${boolToBit(inputs.A)} AND ${boolToBit(inputs.B)} = ${boolToBit(
          logic.Q
        )}`
      : variant === "three"
      ? `Q = ${boolToBit(inputs.A)} AND ${boolToBit(inputs.B)} AND ${boolToBit(
          inputs.C
        )} = ${boolToBit(logic.Q)}`
      : `Q = (${boolToBit(inputs.A)} AND ${boolToBit(
          inputs.B
        )}) AND (${boolToBit(inputs.C)} AND ${boolToBit(
          inputs.D
        )}) AND (${boolToBit(inputs.E)} AND ${boolToBit(inputs.F)}) = ${boolToBit(
          logic.Q
        )}`;
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-2 text-sm text-slate-700">
        ...
        {variant === "six" && (
          <>
            <p>
              <strong>A.B</strong> = A AND B = {boolToBit(logic.AB)}
            </p>
            <p>
              <strong>C.D</strong> = C AND D = {boolToBit(logic.CD)}
            </p>
            <p>
              <strong>E.F</strong> = E AND F = {boolToBit(logic.EF)}
            </p>
          </>
        )}
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-bold text-slate-900">AND Truth Table</h3>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 p-2">
        <svg
          ...
          role="img"
          aria-label="Digital waveform timeline for inputs and output"
        >
        ...
        <main className="flex min-w-0 flex-col gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <svg
              ...
              role="img"
              aria-label="AND logic gate simulation"
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            ...
            <div className="grid gap-2 text-sm text-slate-700">
              ...
              <p>
                ...
                {logic.Q
                  ? "All required inputs are 1, so Q = 1."
                  : "At least one required input is 0, so Q = 0."}
              </p>
            </div>
          </div>
