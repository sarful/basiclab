...
const TABS: readonly { key: DiagramVariant; label: string }[] = [
  { key: "two", label: "2-input XOR" },
  { key: "three", label: "3-input XOR" },
  { key: "six", label: "6-input XOR" },
];
...
function getFormulaTitle(variant: DiagramVariant) {
  if (variant === "two") return "Q = A ⊕ B";
  if (variant === "three") return "Q = A ⊕ B ⊕ C";
...
function stepText(variant: DiagramVariant, step: SimulationStep) {
  if (variant === "two") {
    ...
    if (step === 2) return "The XOR gate checks if A and B are different.";
    if (step === 3) return "The output path carries the XOR result.";
    return "Q is 1 only when A and B are different.";
  }

  if (variant === "three") {
    if (step === 1) return "Inputs A, B, and C are selected.";
    if (step === 2) return "The XOR gate checks odd parity.";
    if (step === 3) return "The output path carries the XOR result.";
    return "Q is 1 when an odd number of inputs are 1.";
  }

  if (step === 1) return "Inputs A-F are selected.";
  if (step === 2) return "A⊕B, C⊕D, and E⊕F are calculated.";
  if (step === 3) return "The final XOR output path moves toward Q.";
  return "Q is 1 when an odd number of all inputs are 1.";
}
...
function TwoInputXorGate({
  ...
  const activeGate = logic.Q && gateStage;
...
function SixInputXorFunction({
  ...
  const finalGateActive = logic.Q && gateStage;
...
function FormulaPanel({ variant, inputs, logic }: FormulaPanelProps) {
  const evaluated =
    variant === "two"
      ? `Q = ${boolToBit(inputs.A)} XOR ${boolToBit(inputs.B)} = ${boolToBit(
          logic.Q
        )}`
      : variant === "three"
      ? `Q = ${boolToBit(inputs.A)} XOR ${boolToBit(inputs.B)} XOR ${boolToBit(
          inputs.C
        )} = ${boolToBit(logic.Q)}`
      : `Q = (${boolToBit(inputs.A)} XOR ${boolToBit(
          inputs.B
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-bold text-slate-900">XOR Truth Table</h3>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 p-2">
        <svg
          ...
          role="img"
          aria-label="Digital waveform timeline for inputs and XOR output"
        >
        ...
        <main className="flex min-w-0 flex-col gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <svg
              ...
              role="img"
              aria-label="XOR logic gate simulation"
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            ...
            <div className="grid gap-2 text-sm text-slate-700">
              ...
              <p>
                ...
                {logic.Q
                  ? "An odd number of required inputs are 1, so XOR output Q = 1."
                  : "An even number of required inputs are 1, so XOR output Q = 0."}
              </p>
            </div>
          </div>
