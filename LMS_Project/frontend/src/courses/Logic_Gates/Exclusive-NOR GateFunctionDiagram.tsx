...
const TABS: readonly { key: DiagramVariant; label: string }[] = [
  { key: "two", label: "2-input XNOR" },
  { key: "three", label: "3-input XNOR" },
  { key: "six", label: "6-input XNOR" },
];
...
function xor2(a: boolean, b: boolean) {
  return a !== b;
}

function xorMany(values: readonly boolean[]) {
...
function calculateLogic(inputs: InputState, variant: DiagramVariant): LogicState {
  const AB = xor2(inputs.A, inputs.B);
  const CD = xor2(inputs.C, inputs.D);
  const EF = xor2(inputs.E, inputs.F);

  const rawXor =
    variant === "two"
      ? AB
      : variant === "three"
      ? xorMany([inputs.A, inputs.B, inputs.C])
      : xorMany([inputs.A, inputs.B, inputs.C, inputs.D, inputs.E, inputs.F]);

  const Q = !rawXor;

  return { AB, CD, EF, rawXor, Q };
}
...
function stepText(variant: DiagramVariant, step: SimulationStep) {
  if (variant === "two") {
    ...
    if (step === 2) return "The XNOR gate checks if A and B are the same.";
    if (step === 3) return "The output path carries the XNOR result.";
    return "Q is 1 when A and B are the same.";
  }

  if (variant === "three") {
    if (step === 1) return "Inputs A, B, and C are selected.";
    if (step === 2) return "The XNOR gate checks even parity.";
    if (step === 3) return "The output path carries the XNOR result.";
    return "Q is 1 when an even number of inputs are 1.";
  }

  if (step === 1) return "Inputs A-F are selected.";
  if (step === 2) return "A⊕B, C⊕D, and E⊕F are calculated.";
  if (step === 3) return "The final XNOR output path moves toward Q.";
  return "Q is 1 when an even number of all inputs are 1.";
}
...
function xorGatePath(x: number, y: number, w: number, h: number) {
  const scaledW = w * SCALE.CIRCUIT_COMPONENT_SCALE;
  const scaledH = h * SCALE.CIRCUIT_COMPONENT_SCALE;

  return [
    `M ${x + 5} ${y}`,
    `C ${x + scaledW * 0.4} ${y} ${x + scaledW * 0.75} ${
      y + scaledH * 0.08
    } ${x + scaledW} ${y + scaledH / 2}`,
    `C ${x + scaledW * 0.75} ${y + scaledH * 0.92} ${
      x + scaledW * 0.4
    } ${y + scaledH} ${x + 5} ${y + scaledH}`,
    `C ${x + scaledW * 0.24} ${y + scaledH * 0.68} ${
      x + scaledW * 0.24
    } ${y + scaledH * 0.32} ${x + 5} ${y}`,
    "Z",
  ].join(" ");
}

function xorExtraCurvePath(x: number, y: number, w: number, h: number) {
  const scaledW = w * SCALE.CIRCUIT_COMPONENT_SCALE;
  const scaledH = h * SCALE.CIRCUIT_COMPONENT_SCALE;

  return [
    `M ${x - 3} ${y + 1}`,
    `C ${x + scaledW * 0.18} ${y + scaledH * 0.32} ${
      x + scaledW * 0.18
    } ${y + scaledH * 0.68} ${x - 3} ${y + scaledH - 1}`,
  ].join(" ");
}
...
function XorGate({ x, y, w, h, symbolSize = 23, active = false }: XorGateProps) {
  const offsetX = COMPONENT_OFFSET.xorGate.x;
  const offsetY = COMPONENT_OFFSET.xorGate.y;

  return (
    <g>
      <path
        d={xorGatePath(x + offsetX, y + offsetY, w, h)}
      ...
      <path
        d={xorExtraCurvePath(x + offsetX, y + offsetY, w, h)}
        fill="none"
        stroke={COLORS.gateStroke}
        strokeWidth={GATE.strokeWidth}
      />
...
function TwoInputXnorGate({
  ...
  const activeGate = gateStage && (logic.rawXor || logic.Q);
  const activeOutput = logic.Q && outputStage;
  const activeTerminal = logic.Q && resultStage;

  return (
    <g
      transform={`translate(${COMPONENT_OFFSET.twoGate.x} ${COMPONENT_OFFSET.twoGate.y})`}
    >
      <InputTerminal
        inputKey="A"
        x={21}
      ...
      <XorGate x={66} y={1} w={79} h={57} symbolSize={15} active={activeGate} />
...
/* -------------------------------------------------------
   3-input XNOR circuit
------------------------------------------------------- */

function ThreeInputXnorGate({
  inputs,
  logic,
  interactive,
  showTruthValues,
  ...
  return (
    <g
      transform={`translate(${COMPONENT_OFFSET.threeGate.x} ${COMPONENT_OFFSET.threeGate.y})`}
    >
      <InputTerminal
        inputKey="A"
        x={25}
      ...
      <XorGate x={72} y={2} w={75} h={58} symbolSize={15} active={activeGate} />
...
/* -------------------------------------------------------
   6-input XNOR circuit
------------------------------------------------------- */

function SixInputXnorFunction({
  inputs,
  logic,
  interactive,
  showTruthValues,
  ...
  const finalGateActive = gateStage && (logic.rawXor || logic.Q);
  ...
  const intermediateRows: readonly {
    ...
    { key: "AB", label: "A⊕B", labelX: 160, labelY: 22, valueX: 198, valueY: 22, path: PATH.six.AB },
    { key: "CD", label: "C⊕D", labelX: 160, labelY: 99, valueX: 198, valueY: 99, path: PATH.six.CD },
    { key: "EF", label: "E⊕F", labelX: 160, labelY: 175, valueX: 198, valueY: 175, path: PATH.six.EF },
  ];

  return (
    <g
      transform={`translate(${COMPONENT_OFFSET.sixFunction.x} ${COMPONENT_OFFSET.sixFunction.y})`}
    >
      {inputRows.map((item) => (
        <InputTerminal
          key={item.key}
      ...
      <XorGate x={75} y={1} w={76} h={58} symbolSize={15} active={firstGateActive.AB} />
      <XorGate x={75} y={78} w={76} h={58} symbolSize={15} active={firstGateActive.CD} />
      <XorGate x={75} y={154} w={76} h={58} symbolSize={15} active={firstGateActive.EF} />
      ...
      <XorGate x={245} y={93} w={84} h={58} symbolSize={15} active={finalGateActive} />
...
function FormulaPanel({ variant, inputs, logic }: FormulaPanelProps) {
  const evaluated =
    variant === "two"
      ? `Q = NOT(${boolToBit(inputs.A)} XOR ${boolToBit(
          inputs.B
        )}) = ${boolToBit(logic.Q)}`
      : variant === "three"
      ? `Q = NOT(${boolToBit(inputs.A)} XOR ${boolToBit(
          inputs.B
        )} XOR ${boolToBit(inputs.C)}) = ${boolToBit(logic.Q)}`
      : `Q = NOT((${boolToBit(inputs.A)} XOR ${boolToBit(
          inputs.B
        )}) XOR (${boolToBit(inputs.C)} XOR ${boolToBit(
          inputs.D
        )}) XOR (${boolToBit(inputs.E)} XOR ${boolToBit(
          inputs.F
        )})) = ${boolToBit(logic.Q)}`;
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-2 text-sm text-slate-700">
        ...
        {variant === "six" && (
          <>
            <p>
              <strong>A⊕B</strong> = A XOR B = {boolToBit(logic.AB)}
            </p>
            <p>
              <strong>C⊕D</strong> = C XOR D = {boolToBit(logic.CD)}
            </p>
            <p>
              <strong>E⊕F</strong> = E XOR F = {boolToBit(logic.EF)}
            </p>
          </>
        )}
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-bold text-slate-900">XNOR Truth Table</h3>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 p-2">
        <svg
          ...
          role="img"
          aria-label="Digital waveform timeline for inputs and XNOR output"
        >
        ...
        <main className="flex min-w-0 flex-col gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <svg
              ...
              role="img"
              aria-label="XNOR logic gate simulation"
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            ...
            <div className="grid gap-2 text-sm text-slate-700">
              ...
              <p>
                ...
                {logic.Q
                  ? "An even number of required inputs are 1, so XNOR output Q = 1."
                  : "An odd number of required inputs are 1, so XNOR output Q = 0."}
              </p>
            </div>
          </div>
