...
const TABS: readonly { key: DiagramVariant; label: string }[] = [
  { key: "two", label: "2-input Universal Gate" },
  { key: "three", label: "3-input Universal Gate" },
  { key: "six", label: "6-input Universal Gate" },
];
...
function calculateLogic(
  ...
  const AB = gateType === "NAND" ? inputs.A && inputs.B : inputs.A || inputs.B;
  const CD = gateType === "NAND" ? inputs.C && inputs.D : inputs.C || inputs.D;
  const EF = gateType === "NAND" ? inputs.E && inputs.F : inputs.E || inputs.F;
...
function getFormulaTitle(variant: DiagramVariant, gateType: UniversalGateType) {
  if (gateType === "NAND") {
...
function stepText(
  ...
  if (variant === "two") {
    ...
    if (step === 2) return `The ${gateType} calculation area checks the input logic.`;
    if (step === 3) return `The output path carries the ${gateType} result.`;
    return `Q shows the final ${gateType} output.`;
  }
  ...
  if (step === 2) {
    return gateType === "NAND"
      ? "A.B, C.D, and E.F are calculated first."
      : "A+B, C+D, and E+F are calculated first.";
  }
  if (step === 3) return `The final ${gateType} output path moves toward Q.`;
  return `Q shows the final ${gateType} output.`;
}
...
function andGatePath(x: number, y: number, w: number, h: number) {
  ...
  return [
    `M ${x} ${y}`,
    `H ${x + scaledW - r}`,
    `A ${r} ${r} 0 0 1 ${x + scaledW - r} ${y + scaledH}`,
    `H ${x}`,
    "Z",
  ].join(" ");
}
...
function orGatePath(x: number, y: number, w: number, h: number) {
  ...
  return [
    `M ${x} ${y}`,
    `C ${x + scaledW * 0.38} ${y} ${x + scaledW * 0.75} ${
      y + scaledH * 0.08
    } ${x + scaledW} ${y + scaledH / 2}`,
...
function UniversalGate({
  ...
  const offsetX = COMPONENT_OFFSET.logicGate.x;
  const offsetY = COMPONENT_OFFSET.logicGate.y;

  const shapePath =
    gateType === "NAND"
      ? andGatePath(x + offsetX, y + offsetY, w, h)
      : orGatePath(x + offsetX, y + offsetY, w, h);
...
function TwoInputUniversalGate({
  inputs,
  logic,
  gateType,
  interactive,
  ...
  const activeGate = gateStage && (logic.raw || logic.Q);
  ...
  return (
    <g transform={`translate(${COMPONENT_OFFSET.twoGate.x} ${COMPONENT_OFFSET.twoGate.y})`}>
      <InputTerminal inputKey="A" x={21} y={16} labelX={0} labelY={21} valueX={31} valueY={13} active={inputs.A} highlighted={activeA} interactive={interactive} showValue={showTruthValues} debug={debug} onToggle={onToggleInput} />
      <InputTerminal inputKey="B" x={21} y={43} labelX={0} labelY={48} valueX={31} valueY={50} active={inputs.B} highlighted={activeB} interactive={interactive} showValue={showTruthValues} debug={debug} onToggle={onToggleInput} />

      {activeA ? <ActiveWire d={PATH.two.inputA} /> : <Wire d={PATH.two.inputA} />}
      {activeB ? <ActiveWire d={PATH.two.inputB} /> : <Wire d={PATH.two.inputB} />}

      <UniversalGate x={66} y={1} w={79} h={57} gateType={gateType} symbolSize={gateType === "NAND" ? 20 : 16} active={activeGate} />
...
function ThreeInputUniversalGate({
  inputs,
  logic,
  gateType,
  interactive,
  ...
  return (
    <g transform={`translate(${COMPONENT_OFFSET.threeGate.x} ${COMPONENT_OFFSET.threeGate.y})`}>
      <InputTerminal inputKey="A" x={25} y={12} labelX={0} labelY={16} valueX={35} valueY={12} active={inputs.A} highlighted={activeA} interactive={interactive} showValue={showTruthValues} debug={debug} onToggle={onToggleInput} />
      <InputTerminal inputKey="B" x={25} y={30} labelX={0} labelY={34} valueX={35} valueY={32} active={inputs.B} highlighted={activeB} interactive={interactive} showValue={showTruthValues} debug={debug} onToggle={onToggleInput} />
      <InputTerminal inputKey="C" x={25} y={49} labelX={0} labelY={53} valueX={35} valueY={52} active={inputs.C} highlighted={activeC} interactive={interactive} showValue={showTruthValues} debug={debug} onToggle={onToggleInput} />
      ...
      <UniversalGate x={72} y={2} w={75} h={58} gateType={gateType} symbolSize={gateType === "NAND" ? 20 : 16} active={activeGate} />
...
function SixInputUniversalFunction({
  inputs,
  logic,
  gateType,
  interactive,
  ...
  const finalGateActive = gateStage && (logic.raw || logic.Q);
  ...
  return (
    <g transform={`translate(${COMPONENT_OFFSET.sixFunction.x} ${COMPONENT_OFFSET.sixFunction.y})`}>
      ...
      <UniversalGate x={75} y={1} w={76} h={58} gateType={gateType} symbolSize={gateType === "NAND" ? 22 : 16} active={firstGateActive.AB} />
      <UniversalGate x={75} y={78} w={76} h={58} gateType={gateType} symbolSize={gateType === "NAND" ? 22 : 16} active={firstGateActive.CD} />
      <UniversalGate x={75} y={154} w={76} h={58} gateType={gateType} symbolSize={gateType === "NAND" ? 22 : 16} active={firstGateActive.EF} />
      ...
      <UniversalGate x={245} y={93} w={84} h={58} gateType={gateType} symbolSize={gateType === "NAND" ? 22 : 16} active={finalGateActive} />
      ...
      <Caption x={76} y={240}>
        6 input “{gateType}” universal function
      </Caption>
    </g>
  );
}
...
function UniversalGateSelector({
  value,
  onChange,
}: {
  value: UniversalGateType;
  onChange: (gateType: UniversalGateType) => void;
}) {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <ControlButton
          active={value === "NAND"}
          ariaLabel="Select NAND universal gate"
          onClick={() => onChange("NAND")}
        >
          NAND
        </ControlButton>
        ...
        <ControlButton
          active={value === "NOR"}
          ariaLabel="Select NOR universal gate"
          onClick={() => onChange("NOR")}
        >
          NOR
        </ControlButton>
      </div>
...
function StatusBadge({ active, gateType }: StatusBadgeProps) {
  return (
    <span
      role="status"
      aria-live="polite"
...
function FormulaPanel({ variant, inputs, logic, gateType }: FormulaPanelProps) {
  const operatorWord = gateType === "NAND" ? "AND" : "OR";

  const evaluated =
    variant === "two"
      ? `Q = NOT(${boolToBit(inputs.A)} ${operatorWord} ${boolToBit(
          inputs.B
        )}) = ${boolToBit(logic.Q)}`
      : variant === "three"
      ? `Q = NOT(${boolToBit(inputs.A)} ${operatorWord} ${boolToBit(
          inputs.B
        )} ${operatorWord} ${boolToBit(inputs.C)}) = ${boolToBit(logic.Q)}`
      : `Q = NOT((${boolToBit(inputs.A)} ${operatorWord} ${boolToBit(
          inputs.B
        )}) ${operatorWord} (${boolToBit(inputs.C)} ${operatorWord} ${boolToBit(
          inputs.D
        )}) ${operatorWord} (${boolToBit(inputs.E)} ${operatorWord} ${boolToBit(
          inputs.F
        )})) = ${boolToBit(logic.Q)}`;
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-2 text-sm text-slate-700">
        <p className="font-semibold text-slate-900">
          {getFormulaTitle(variant, gateType)}
        </p>

        {variant === "six" && (
          <>
            <p>
              <strong>{gateType === "NAND" ? "A.B" : "A+B"}</strong> ={" "}
              {boolToBit(logic.AB)}
            </p>
            <p>
              <strong>{gateType === "NAND" ? "C.D" : "C+D"}</strong> ={" "}
              {boolToBit(logic.CD)}
            </p>
            <p>
              <strong>{gateType === "NAND" ? "E.F" : "E+F"}</strong> ={" "}
              {boolToBit(logic.EF)}
            </p>
          </>
        )}
...
function TruthTable({ variant, inputs, gateType }: TruthTableProps) {
  ...
  const rows = useMemo(() => {
    return Array.from({ length: rowCount }, (_, index) => {
      ...
      const rowLogic = calculateLogic(rowInputs, variant, gateType);
  ...
  }, [keys, rowCount, variant, gateType]);
    ...
    const header =
      variant === "six"
        ? [
            ...
            gateType === "NAND" ? "C.D" : "C+D",
            gateType === "NAND" ? "E.F" : "E+F",
            "Q",
          ].join("\t")
        : [...keys, "Q"].join("\t");
...
function WaveformPanel({ variant, samples, gateType }: WaveformPanelProps) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">
          {gateType} · {samples.length} samples
        </span>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 p-2">
        <svg
          ...
          role="img"
          aria-label={`Digital waveform timeline for inputs and ${gateType} output`}
        >
...
function LeftControlPanel({
  ...
  onToggleInput: (key: InputKey) => void;
  onPreset: (preset: PresetType) => void;
  onGateTypeChange: (gateType: UniversalGateType) => void;
  ...
  return (
    <aside className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <StatusBadge active={logic.Q} gateType={gateType} />
      </div>

      <UniversalGateSelector value={gateType} onChange={onGateTypeChange} />
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
        ...
        <div className="flex flex-wrap gap-2">
          {variant === "six" && (
            <>
              <StatePill label={gateType === "NAND" ? "A.B" : "A+B"} value={logic.AB} />
              <StatePill label={gateType === "NAND" ? "C.D" : "C+D"} value={logic.CD} />
              <StatePill label={gateType === "NAND" ? "E.F" : "E+F"} value={logic.EF} />
            </>
          )}
...
export default function AndFunctionDiagram({
  ...
  const [selectedGateType, setSelectedGateType] =
    useState<UniversalGateType>(gateType);
  ...
  const logic = useMemo(
    () => calculateLogic(inputs, currentVariant, selectedGateType),
    [inputs, currentVariant, selectedGateType]
  );
      ...
      <div className="grid gap-4 lg:grid-cols-[340px_minmax(0,1fr)]">
        {showControls && (
          <LeftControlPanel
            ...
            onGateTypeChange={setSelectedGateType}
        ...
        <main className="flex min-w-0 flex-col gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <svg
              ...
              role="img"
              aria-label={`${selectedGateType} universal logic gate simulation`}
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              ...
              <StatusBadge active={logic.Q} gateType={selectedGateType} />
            </div>

            <div className="grid gap-2 text-sm text-slate-700">
              <p>
                <strong>Universal gate:</strong> {selectedGateType}
              </p>
              <p>
                <strong>Formula:</strong>{" "}
                {getFormulaTitle(currentVariant, selectedGateType)}
              </p>
              <p>
              ...
              <p>
                ...
                {logic.Q
                  ? `${selectedGateType} output Q = 1.`
                  : `${selectedGateType} output Q = 0.`}
              </p>
              <p className="text-xs text-slate-500">
                NAND and NOR are called universal gates because digital logic
                circuits can be built using only NAND gates or only NOR gates.
              </p>
            </div>
          </div>
