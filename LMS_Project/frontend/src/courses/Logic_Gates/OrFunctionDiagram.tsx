...
const TABS: readonly { key: DiagramVariant; label: string }[] = [
  { key: "two", label: "2-input OR" },
  { key: "three", label: "3-input OR" },
  { key: "six", label: "6-input OR" },
];
...
function stepText(variant: DiagramVariant, step: SimulationStep) {
  if (variant === "two") {
    ...
    if (step === 2) return "The OR gate checks A + B.";
    if (step === 3) return "The output path carries the result.";
    return "Q shows the final OR output.";
  }

  if (variant === "three") {
    if (step === 1) return "Inputs A, B, and C are selected.";
    if (step === 2) return "The OR gate checks A + B + C.";
    if (step === 3) return "The output path carries the result.";
    return "Q shows the final OR output.";
  }
...
function FormulaPanel({ variant, inputs, logic }: FormulaPanelProps) {
  const evaluated =
    variant === "two"
      ? `Q = ${boolToBit(inputs.A)} OR ${boolToBit(inputs.B)} = ${boolToBit(
          logic.Q
        )}`
      : variant === "three"
      ? `Q = ${boolToBit(inputs.A)} OR ${boolToBit(inputs.B)} OR ${boolToBit(
          inputs.C
        )} = ${boolToBit(logic.Q)}`
      : `Q = (${boolToBit(inputs.A)} OR ${boolToBit(
          inputs.B
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-bold text-slate-900">OR Truth Table</h3>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 p-2">
        <svg
          ...
          role="img"
          aria-label="Digital waveform timeline for inputs and OR output"
        >
        ...
        <main className="flex min-w-0 flex-col gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <svg
              ...
              role="img"
              aria-label="OR logic gate simulation"
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            ...
            <div className="grid gap-2 text-sm text-slate-700">
              ...
              <p>
                ...
                {logic.Q
                  ? "At least one required input is 1, so OR output Q = 1."
                  : "All required inputs are 0, so OR output Q = 0."}
              </p>
            </div>
          </div>
