...
const TABS: readonly { key: DiagramVariant; label: string }[] = [
  { key: "two", label: "2-input NAND" },
  { key: "three", label: "3-input NAND" },
  { key: "six", label: "6-input NAND" },
];
...
function stepText(variant: DiagramVariant, step: SimulationStep) {
  if (variant === "two") {
    ...
    if (step === 2) return "The NAND calculation checks NOT(A · B).";
    if (step === 3) return "The output path carries the NAND result.";
    return "Q shows the final NAND output.";
  }

  if (variant === "three") {
    if (step === 1) return "Inputs A, B, and C are selected.";
    if (step === 2) return "The NAND calculation checks NOT(A · B · C).";
    if (step === 3) return "The output path carries the NAND result.";
    return "Q shows the final NAND output.";
  }
  ...
  if (step === 3) return "The final NAND output path moves toward Q.";
  return "Q shows the final NAND output.";
}
...
function TwoInputNandGate({
  ...
  const activeGate = gateStage && (logic.rawAnd || logic.Q);
...
/* -------------------------------------------------------
   6-input NAND circuit
------------------------------------------------------- */

function SixInputNandFunction({
  inputs,
  logic,
  interactive,
  showTruthValues,
  ...
  const finalGateActive = gateStage && (logic.rawAnd || logic.Q);
...
function FormulaPanel({ variant, inputs, logic }: FormulaPanelProps) {
  const evaluated =
    variant === "two"
      ? `Q = NOT(${boolToBit(inputs.A)} AND ${boolToBit(
          inputs.B
        )}) = ${boolToBit(logic.Q)}`
      : variant === "three"
      ? `Q = NOT(${boolToBit(inputs.A)} AND ${boolToBit(inputs.B)} AND ${boolToBit(
          inputs.C
        )}) = ${boolToBit(logic.Q)}`
      : `Q = NOT((${boolToBit(inputs.A)} AND ${boolToBit(
          inputs.B
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-bold text-slate-900">NAND Truth Table</h3>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 p-2">
        <svg
          ...
          role="img"
          aria-label="Digital waveform timeline for inputs and NAND output"
        >
        ...
        <main className="flex min-w-0 flex-col gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <svg
              ...
              role="img"
              aria-label="NAND logic gate simulation"
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            ...
            <div className="grid gap-2 text-sm text-slate-700">
              ...
              <p>
                ...
                {logic.Q
                  ? "At least one required input is 0, so NAND output Q = 1."
                  : "All required inputs are 1, so NAND output Q = 0."}
              </p>
            </div>
          </div>
