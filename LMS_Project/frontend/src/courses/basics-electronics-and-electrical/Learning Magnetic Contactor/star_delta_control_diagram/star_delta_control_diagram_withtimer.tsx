...
import OnDelayNCContactSymbol from "../../../../library/contacts/OnDelayNCContactSymbol";
import OnDelayNOContactSymbol from "../../../../library/contacts/OnDelayNOContactSymbol";
...
const TIMER_NO_CONTACT_TOP_X = 20;
...
function ActivePathLayer({
  ...
  startPathState: WireState;
  holdState: WireState;
  overloadState: WireState;
  offState: WireState;
  mainCoilState: WireState;
  ...
  return (
    <>
      ...
      <ActivePath d={PATH.onPushPath} state={startPathState} />
      ...
      <ActivePath d={PATH.mainCoilPath} state={mainCoilState} />
...
function FlowLayer({
  ...
  const sourceFlowActive =
    startPathState === "active" ||
    holdState === "active" ||
    mainCoilState === "active" ||
    timerState === "active" ||
    starState === "active" ||
    deltaState === "active";

  const commonBusFlowActive =
    mainCoilState === "active" ||
    timerState === "active" ||
    starState === "active" ||
    deltaState === "active";

  const commonBranchBusPath = `M ${G.k1BranchBusStartX} ${G.k1BranchBusY} H ${G.k1BranchBusEndX}`;
  ...
  return (
    <>
      ...
      <ElectronFlow
        path={commonBranchBusPath}
        active={commonBusFlowActive}
        duration={2.35}
      />

      <ElectronFlow
        path={PATH.onPushPath}
        active={startPathState === "active"}
        duration={2.1}
      />
      ...
      <ElectronFlow
        ...
        active={starState === "active"}
        duration={2.5}
      />

      <ElectronFlow
        path={PATH.deltaRungPath}
        active={deltaState === "active"}
        duration={2.5}
      />
...
function ProtectionAndStartSection({
  ...
  const k1AuxScale = getVerticalContactScale(G.holdBranchY, G.holdReturnY);
  const k1AuxPosition = getAnchoredSymbolPosition(
    G.holdBranchEndX,
    G.holdBranchY,
    k1AuxScale,
    AUX_CONTACT_TOP_X,
...
function MainTimerStarSection({
  ...
  const t1NcScale = getVerticalContactScale(
    G.t1NcTopWireEndY,
    G.t1NcBottomWireStartY,
  );
  const t1NcPosition = getAnchoredSymbolPosition(
    G.t1NcFeedX,
    G.t1NcTopWireEndY,
    t1NcScale,
    AUX_CONTACT_TOP_X,
  ...
  const k2NcScale = getVerticalContactScale(
    G.k2NcTopWireEndY,
    G.k2NcBottomWireStartY,
  );
...
function DeltaSection({
  ...
  const t1NoScale = getVerticalContactScale(
    G.t1NoTopWireEndY,
    G.t1NoBottomWireStartY,
  );
  const t1NoPosition = getAnchoredSymbolPosition(
    G.t1NoFeedX,
    G.t1NoTopWireEndY,
    t1NoScale,
    TIMER_NO_CONTACT_TOP_X,
  );

  const k2NoScale = getVerticalContactScale(G.k2NoTopLinkY, G.k2NoBottomLinkY);
  const k2NoPosition = getAnchoredSymbolPosition(
    G.k2NoTopLinkEndX,
    G.k2NoTopLinkY,
    k2NoScale,
    AUX_CONTACT_TOP_X,
  ...
  const k3NcScale = getVerticalContactScale(
    G.k3NcTopWireEndY,
    G.k3NcBottomWireStartY,
  );
  const k3NcPosition = getAnchoredSymbolPosition(
    G.k3NcFeedX,
    G.k3NcTopWireEndY,
    k3NcScale,
    AUX_CONTACT_TOP_X,
...
export default function StarDeltaControlDiagramWithTimer({
  ...
  const timerDone = transferOpen || deltaOn || (timerOn && !starOn);

  const healthy = mcbOn && !overloadTripped;

  const startPathCurrent = healthy && startPressed;
  const holdPathCurrent = healthy && mainOn && !startPressed;

  const mainCoilCurrent = startPathCurrent || holdPathCurrent;
  const timerCoilCurrent = healthy && timerOn;

  const starCoilCurrent = healthy && mainOn && timerOn && starOn && !deltaOn;
  const deltaCoilCurrent = healthy && mainOn && deltaOn && !starOn;

  const commonControlCurrent =
    healthy &&
    (startPathCurrent ||
      holdPathCurrent ||
      mainCoilCurrent ||
  ...
  const neutralCurrent =
    mainCoilCurrent || timerCoilCurrent || starCoilCurrent || deltaCoilCurrent;

  const upstreamState: WireState = commonControlCurrent ? "active" : "inactive";
  const startPathState: WireState = startPathCurrent ? "active" : "inactive";
  const holdState: WireState = holdPathCurrent ? "active" : "inactive";
  const mainCoilState: WireState = mainCoilCurrent ? "active" : "inactive";
  const timerState: WireState = timerCoilCurrent ? "active" : "inactive";
  const starState: WireState = starCoilCurrent ? "active" : "inactive";
  const deltaState: WireState = deltaCoilCurrent ? "active" : "inactive";
  const overloadState: WireState = commonControlCurrent ? "active" : "inactive";
  const offState: WireState = commonControlCurrent ? "active" : "inactive";
  const neutralState: WireState = neutralCurrent ? "active" : "inactive";
            ...
            <g transform={`translate(${BOARD.x}, ${BOARD.y})`}>
              ...
              <g transform={canvasTransform}>
                <ActivePathLayer
                  ...
                  startPathState={startPathState}
                  holdState={holdState}
                  overloadState={overloadState}
                  offState={offState}
                  mainCoilState={mainCoilState}
