...
import ACMotor3P6Terminal from "../../Project/library/motors/ACMotor3P6Terminal";
...
function FlowLayer({
  ...
  mainPowerCurrent: boolean;
  starPowerCurrent: boolean;
  deltaPowerCurrent: boolean;
  tripState: WireState;
}) {
  return (
    <>
...
export default function StarDeltaPowerDiagramWithTimer({
  ...
  const illegalStarDeltaOverlap = healthy && starOn && deltaOn;

  const starPowerCurrent =
    healthy && mainOn && starOn && !deltaOn && !transferOpen;

  const deltaPowerCurrent =
    healthy && mainOn && deltaOn && !starOn && !transferOpen;

  const mainPowerCurrent = starPowerCurrent || deltaPowerCurrent;

  const mainState: WireState =
    illegalStarDeltaOverlap || (healthy && mainOn && (starOn || deltaOn))
      ? illegalStarDeltaOverlap
        ? "fault"
        : "active"
      : "inactive";

  const starState: WireState =
    illegalStarDeltaOverlap || starPowerCurrent
      ? illegalStarDeltaOverlap
        ? "fault"
        : "active"
      : "inactive";
  ...
  const deltaState: WireState =
    illegalStarDeltaOverlap || deltaPowerCurrent
      ? illegalStarDeltaOverlap
        ? "fault"
        : "active"
      : "inactive";

  const tripState: WireState =
    mcbOn && (overloadTripped || illegalStarDeltaOverlap)
      ? "fault"
      : "inactive";
          ...
          >
            ...
            <g transform={`translate(${BOARD.x}, ${BOARD.y})`}>
              ...
              <g transform={canvasTransform}>
                ...
                <FlowLayer
                  mainPowerCurrent={mainPowerCurrent}
                  starPowerCurrent={starPowerCurrent}
                  deltaPowerCurrent={deltaPowerCurrent}
                  tripState={tripState}
                />
                ...
                <MotorSection
                  ...
                  motorMode={motorMode}
                  loadPercent={loadPercent}
                  overloadTripped={overloadTripped || illegalStarDeltaOverlap}
                  transferOpen={transferOpen}
                  deltaOn={deltaPowerCurrent}
                  starOn={starPowerCurrent}
                  mainOn={mainOn}
                />
