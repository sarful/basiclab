"use client";

export function ZenerHero() {
  return (
    <section className="rounded-3xl border bg-white p-4 shadow-sm sm:p-6">
      <div className="grid gap-4 lg:grid-cols-[1.4fr_0.6fr] lg:items-center">
        <div>
          <p className="text-sm font-black uppercase tracking-widest text-purple-700">
            Electronics Learning
          </p>
          <h1 className="mt-2 text-3xl font-black text-slate-900 sm:text-4xl">
            à¦œà§‡à¦¨à¦¾à¦° à¦¡à¦¾à¦¯à¦¼à§‹à¦¡ (Zener Diode)
          </h1>
          <p className="mt-2 text-sm leading-6 text-slate-600 sm:text-base sm:leading-7">
            Zener diode reverse bias-à¦ à¦¨à¦¿à¦°à§à¦¦à¦¿à¦·à§à¦Ÿ voltage clamp à¦•à¦°à§‡à¥¤ à¦¨à¦¿à¦šà§‡à¦° control panel à¦¦à¦¿à§Ÿà§‡ voltage, Vz, bias mode à¦à¦¬à¦‚ animation à¦¨à¦¿à§Ÿà¦¨à§à¦¤à§à¦°à¦£ à¦•à¦°à§à¦¨à¥¤
          </p>
        </div>
        <div className="rounded-3xl bg-slate-900 p-4 text-white sm:p-5">
          <p className="text-xs font-bold text-slate-300">Key Idea</p>
          <p className="mt-2 text-xl font-black sm:text-2xl">
            Reverse voltage â‰¥ Vz à¦¹à¦²à§‡ breakdown active à¦¹à§Ÿà¥¤
          </p>
        </div>
      </div>
    </section>
  );
}
