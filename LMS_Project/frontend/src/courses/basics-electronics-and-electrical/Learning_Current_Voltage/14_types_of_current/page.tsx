...
import CurrentVoltageLessonFourSimulation from "./CurrentVoltageLessonFourSimulation";
...
function InProgressTab({ title }: { title: string }) {
  return (
    ...
    >
      ...
      >
        This section is the next part of the Types of Current lesson build. The
        shared lesson shell and course navigation are now ready, and this tab is
        prepared for the next content pass.
      </p>
    </section>
  );
}

export default function CurrentVoltageLessonFourteenTypesOfCurrentEmbeddedPage() {
  const { activeTab, setActiveTab, tabs } = useAuthorizedLessonTabs<LessonFourteenTab>(
    [
    ...
    ] satisfies LessonShellTab<LessonFourteenTab>[],
    "lesson",
  );
        ...
        >
          ...
          <div style={{ padding: "28px 22px 22px", background: "#f8fafc" }}>
            <div style={{ marginBottom: 22 }}>
              <CurrentVoltageCourseNav currentLessonId={14} />
            </div>

            {activeTab === "lesson" ? <CurrentVoltageLessonFourSimulation /> : null}
