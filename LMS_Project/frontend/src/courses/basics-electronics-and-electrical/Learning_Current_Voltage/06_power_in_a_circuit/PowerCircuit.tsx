  ...
  return (
    <g>
      {Array.from({ length: ELECTRON_COUNT }, (_, particle) => {
        ...
        return (
          <circle
            key={`charge-particle-${particle}-${active ? "running" : "paused"}`}
            r={FLOW.radius}
            fill={STYLE.flow}
            opacity={0}
          >
...
export function PowerCircuit({
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="What is power in a circuit"
    >
      <defs>
        <filter
          id="electricityGlow"
      ...
      <g transform={canvasTransform}>
        ...
        <text
          ...
          fill={STYLE.muted}
        >
          Voltage pushes charge - Current carries energy - Power ={" "}
          {power.toFixed(2)}W - Flow level: {flowLevel}
        </text>
