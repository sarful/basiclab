...
import CurrentVoltageLessonSixPowerSimulation from "./CurrentVoltageLessonSixPowerSimulation";
...
function InProgressTab({ title }: { title: string }) {
  return (
    ...
    >
      ...
      >
        This section is the next part of the Power in a Circuit lesson build.
...
export default function CurrentVoltageLessonSixPowerEmbeddedPage() {
  const { activeTab, setActiveTab, tabs } = useAuthorizedLessonTabs<LessonSixTab>(
    [
    ...
    ] satisfies LessonShellTab<LessonSixTab>[],
    "lesson",
  );
        ...
        >
          ...
          <div style={{ padding: "28px 22px 22px", background: "#f8fafc" }}>
            <div style={{ marginBottom: 22 }}>
              <CurrentVoltageCourseNav currentLessonId={6} />
            </div>

            {activeTab === "lesson" ? <CurrentVoltageLessonSixPowerSimulation /> : null}
