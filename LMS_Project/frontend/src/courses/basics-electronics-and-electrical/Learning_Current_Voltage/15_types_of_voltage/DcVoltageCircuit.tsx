...
function getParticleDuration(current: number) {
  return Math.max(0.8, 2.4 - current / 2);
}
  ...
  return (
    <g>
      {Array.from({ length: ELECTRON_COUNT }, (_, particle) => {
        ...
        return (
          <circle
            key={`dc-particle-${particle}-${isPlaying ? "running" : "paused"}`}
            r={FLOW.radius}
            fill={STYLE.flow}
            opacity={0}
          >
...
export function DcCircuit({ voltage, current, isPlaying }: DcCircuitProps) {
  const particleDuration = getParticleDuration(current);
    ...
    >
      ...
      <g transform={canvasTransform}>
        ...
        <text
          ...
          fill={STYLE.muted}
        >
          Battery provides steady DC voltage - Current = {current.toFixed(2)}A
        </text>
