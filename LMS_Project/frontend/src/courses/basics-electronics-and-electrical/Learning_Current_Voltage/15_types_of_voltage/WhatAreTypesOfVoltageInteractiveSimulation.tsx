...
export default function WhatAreTypesOfVoltageInteractiveSimulation() {
  ...
  const [dcResistance, setDcResistance] = useState<number>(DEFAULT_DC_RESISTANCE);
  const [acPeakVoltage, setAcPeakVoltage] = useState<number>(DEFAULT_AC_PEAK_VOLTAGE);
  const [acFrequency, setAcFrequency] = useState<number>(DEFAULT_AC_FREQUENCY);
  const [acResistance, setAcResistance] = useState<number>(DEFAULT_AC_RESISTANCE);

  const dcCurrent = useMemo(() => calculateCurrent(dcVoltage, dcResistance), [dcVoltage, dcResistance]);
  const acRmsVoltage = useMemo(() => calculateRms(acPeakVoltage), [acPeakVoltage]);
  const acRmsCurrent = useMemo(() => calculateCurrent(acRmsVoltage, acResistance), [acRmsVoltage, acResistance]);
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-amber-200 bg-white px-4 py-3 text-sm">
                ...
                <span className="flex items-center gap-2 text-red-700">
                  <span className="h-1 w-10 rounded-full bg-red-500" /> Voltage (V)
                </span>
                <span className="flex items-center gap-2 text-blue-700">
                  <span className="h-1 w-10 rounded-full bg-blue-500" /> Current (I)
                </span>
              </div>
            </div>
          </section>
