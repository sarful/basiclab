  ...
  return (
    <g>
      <svg
        ...
        overflow="visible"
        style={{
          filter: current > 0.25 ? "url(#currentLedGlow)" : "none",
        }}
      >
        <g
  ...
  return (
    <g>
      {Array.from({ length: ELECTRON_COUNT }, (_, particle) => {
        ...
        return (
          <circle
            key={`current-particle-${particle}-${active ? "running" : "paused"}`}
            r={FLOW.radius}
            fill={STYLE.flow}
            opacity={0}
          >
...
function StrengthBar({ currentPercent }: { currentPercent: number }) {
  return (
    <g>
      <rect
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Current means moving electric charge through a complete circuit.
          </p>
        </div>
      ...
      <div className="p-4">
        <svg
          ...
          role="img"
          aria-label="Current flow circuit simulation"
        >
          <defs>
            <filter
              id="currentLedGlow"
          ...
          <g transform={canvasTransform}>
            ...
            >
              ...
              {current.toFixed(2)}A - Flow level: {currentLevel}
            </text>
            ...
            <g fill="none" strokeLinecap="round" strokeLinejoin="round">
              <WirePath
                points={WIRE.positiveInput}
                stroke={STYLE.positiveWire}
              />
              <WirePath
                points={WIRE.positiveOutput}
                stroke={STYLE.positiveWire}
              />
            ...
            <CurrentParticles
              active={effectivePlaying}
              duration={particleDuration}
            />

            <StrengthBar currentPercent={currentPercent} />
          </g>
        </svg>
      </div>
    </div>
