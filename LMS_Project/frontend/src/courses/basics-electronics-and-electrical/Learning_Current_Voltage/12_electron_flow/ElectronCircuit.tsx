...
const LABEL = {
  ...
  conventional: { x: 224, y: 124 },
  electron: { x: 426, y: 296 },
  ...
  return (
    <g>
      {Array.from({ length: ELECTRON_COUNT }, (_, particle) => {
        ...
        return (
          <circle
            key={`charge-particle-${particle}-${directionMode}-${
...
export function ElectronCircuit({
  ...
  directionMode,
  isPlaying,
}: ElectronCircuitProps) {
  const isElectronMode = directionMode === "electron";

  const particleDuration = getElectronSpeed(flowPercent);
  const particleColor = isElectronMode
    ? STYLE.electronFlow
    : STYLE.conventionalFlow;

  const activeFlowPath = isElectronMode
    ? PATH.electronFlow
    : PATH.conventionalFlow;
  ...
  return (
    ...
    >
      ...
      <g transform={canvasTransform}>
        ...
        <text
          x={LABEL.conventional.x}
          y={LABEL.conventional.y}
          fontSize="14"
          fontWeight="700"
          fill={STYLE.conventionalFlow}
        >
          Conventional →
        </text>
        ...
        <StrengthBar flowPercent={flowPercent} color={particleColor} />
      </g>
    </svg>
  );
}
