...
import CurrentVoltageLessonElevenComparisonSimulation from "./CurrentVoltageLessonElevenComparisonSimulation";
...
function InProgressTab({ title }: { title: string }) {
  return (
    ...
    >
      ...
      >
        This section is the next part of the Series vs Parallel Comparison
        lesson build. The shared lesson shell and course navigation are now
        ready, and this tab is prepared for the next content pass.
      </p>
    </section>
  );
}

export default function CurrentVoltageLessonElevenComparisonEmbeddedPage() {
  const { activeTab, setActiveTab, tabs } = useAuthorizedLessonTabs<LessonElevenTab>(
    [
    ...
    ] satisfies LessonShellTab<LessonElevenTab>[],
    "lesson",
  );
        ...
        >
          ...
          <div style={{ padding: "28px 22px 22px", background: "#f8fafc" }}>
            <div style={{ marginBottom: 22 }}>
              <CurrentVoltageCourseNav currentLessonId={11} />
            </div>

            {activeTab === "lesson" ? (
              <CurrentVoltageLessonElevenComparisonSimulation />
            ) : null}
