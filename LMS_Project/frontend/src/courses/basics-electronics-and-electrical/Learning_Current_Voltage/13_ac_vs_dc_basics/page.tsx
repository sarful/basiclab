...
import CurrentVoltageLessonThirteenAcVsDcSimulation from "./CurrentVoltageLessonThirteenAcVsDcSimulation";
...
function InProgressTab({ title }: { title: string }) {
  return (
    ...
    >
      ...
      >
        This section is the next part of the AC vs DC Basics lesson build. The
...
export default function CurrentVoltageLessonThirteenAcVsDcEmbeddedPage() {
  const { activeTab, setActiveTab, tabs } = useAuthorizedLessonTabs<LessonThirteenTab>(
    [
    ...
    ] satisfies LessonShellTab<LessonThirteenTab>[],
    "lesson",
  );
        ...
        >
          ...
          <div style={{ padding: "28px 22px 22px", background: "#f8fafc" }}>
            <div style={{ marginBottom: 22 }}>
              <CurrentVoltageCourseNav currentLessonId={13} />
            </div>

            {activeTab === "lesson" ? (
              <CurrentVoltageLessonThirteenAcVsDcSimulation />
            ) : null}
