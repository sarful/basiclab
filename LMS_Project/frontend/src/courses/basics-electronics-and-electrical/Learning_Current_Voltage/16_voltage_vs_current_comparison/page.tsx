...
import CurrentVoltageLessonSevenSimulation from "./CurrentVoltageLessonSevenSimulation";
...
function InProgressTab({ title }: { title: string }) {
  return (
    ...
    >
      ...
      >
        This section is the next part of the Voltage vs Current Comparison lesson
        build. The shared lesson shell and course navigation are now ready, and
        this tab is prepared for the next content pass.
      </p>
    </section>
  );
}

export default function CurrentVoltageLessonSixteenVoltageVsCurrentComparisonEmbeddedPage() {
  const { activeTab, setActiveTab, tabs } = useAuthorizedLessonTabs<LessonSixteenTab>(
    [
    ...
    ] satisfies LessonShellTab<LessonSixteenTab>[],
    "lesson",
  );
        ...
        >
          ...
          <div style={{ padding: "28px 22px 22px", background: "#f8fafc" }}>
            <div style={{ marginBottom: 22 }}>
              <CurrentVoltageCourseNav currentLessonId={16} />
            </div>

            {activeTab === "lesson" ? <CurrentVoltageLessonSevenSimulation /> : null}
