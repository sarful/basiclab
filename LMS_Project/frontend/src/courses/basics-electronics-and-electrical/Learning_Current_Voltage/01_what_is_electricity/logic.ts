...
export function getElectricityDefinition(
  voltage: number,
  resistance: number,
): string {
  const current = calculateCurrent(voltage, resistance);
  const level = getFlowLevel(current).toLowerCase();
  return `Electricity is the movement of electric charge. Here, ${voltage.toFixed(1)}V with ${resistance.toFixed(1)} Ohm creates ${current.toFixed(2)}A current, so charge flow is ${level}.`;
}

export const electricityLogicTests = [
  { voltage: 12, resistance: 6, current: 2 },
  { voltage: 24, resistance: 6, current: 4 },
  { voltage: 6, resistance: 12, current: 0.5 },
] as const;

export const electricityLevelTests = [
