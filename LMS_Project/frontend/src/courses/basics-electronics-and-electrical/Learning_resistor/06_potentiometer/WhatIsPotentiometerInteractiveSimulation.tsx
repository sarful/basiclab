...
export default function WhatIsPotentiometerInteractiveSimulation({
  embedded = false,
}: {
  embedded?: boolean;
}) {
  ...
  const wiperVoltage =
    mode === "voltageDivider" ? outputVoltage : supplyVoltage;
  ...
  const power =
    activeCurrent *
    activeCurrent *
    (mode === "voltageDivider" ? totalResistance : rheostatResistance);
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-2">
            <div className="grid gap-4 md:grid-cols-4">
              <MetricCard
                label="Wiper Voltage"
                value={formatNumber(wiperVoltage, 2)}
                unit="V"
              ...
              <MetricCard
                label="Current"
                value={formatCurrent(activeCurrent)
                  .replace(" A", "")
                  .replace(" mA", "")}
                unit={activeCurrent >= 1 ? "A" : "mA"}
                tone="text-green-600"
              />
              <div className={`rounded-2xl border p-4 shadow-sm ${status.bg}`}>
                ...
                <p className={`mt-2 text-xl font-bold ${status.tone}`}>
                  {status.label}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 shadow-sm">
                ...
                <p className="mt-1 text-xs text-slate-600">
                  Usually connected to Ground / 0V
                </p>
              </div>
              <div className="rounded-2xl border border-purple-100 bg-purple-50 p-4 shadow-sm">
                ...
                <p className="mt-1 text-xs text-slate-600">
                  Changes with knob position
                </p>
              </div>
              <div className="rounded-2xl border border-green-100 bg-green-50 p-4 shadow-sm">
                <p className="text-xs uppercase tracking-[0.18em] text-green-600">
                  Terminal 3 Voltage
                </p>
