  ...
  return (
    <div className={CONTROL_STYLE.wrapper}>
      <div className="mb-4 rounded-2xl bg-slate-950 p-4 text-white">
        ...
        <h3 className="mt-1 text-xl font-black">Optocoupler Working</h3>
        <div className="mt-3 flex flex-wrap gap-2 text-xs font-black">
          <span
            className={
              isInputOn
                ? "rounded-full bg-emerald-400 px-3 py-1 text-slate-950"
                : "rounded-full bg-white/10 px-3 py-1 text-white"
            }
          >
            Input {isInputOn ? "ON" : "OFF"}
          </span>
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button
              ...
              aria-pressed={controlMode === "onOff"}
            >
              ON/OFF Mode
            </button>
            <button
              ...
              aria-pressed={controlMode === "timeline"}
            >
              Timeline Mode
            </button>
          </div>
...
export default function WhatIsOptocouplerInteractive({
  ...
  const visualState: VisualState = useMemo(() => {
    ...
    const inputCurrentActive = isInputOn && isVisualActive && simulationStep === 0;
    const ledCurrentActive = isInputOn && isVisualActive && simulationStep === 1;
  ...
  }, [controlMode, isInputOn, isRunning, simulationStep]);
      ...
      <div className="min-w-0 rounded-[28px] border border-slate-200 bg-slate-50/70 p-4 sm:p-5">
        <section className="mb-4 rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() => handleControlModeChange("onOff")}
            ...
            <button
              type="button"
              onClick={() => handleControlModeChange("timeline")}
              className={`rounded-xl px-3 py-3 text-sm font-black transition ${
                controlMode === "timeline"
                  ? "bg-blue-700 text-white shadow-sm"
                  : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
