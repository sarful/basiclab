...
const TIMELINE_STEPS = [
  ...
  },
  {
    ...
    description:
      "Light crosses the isolation gap and reaches the phototransistor base region.",
  },
  {
    id: 3,
    label: "Output ON",
    title: "Phototransistor conducts",
    description:
      "The phototransistor switches on and output current flows through the load.",
  },
...
type PhototransistorCircuitControlPanelProps = {
...
function calculatePhototransistorSimulation({
  enabled,
  inputVoltage,
  isolation,
}: {
  ...
  const currentTransferRatioPercent = 80;
  ...
  const outputCurrent = clamp(
    (ledCurrent * currentTransferRatioPercent) / 100,
    0,
    50,
  );

  const transferPercent =
    ledCurrent > 0 ? clamp((outputCurrent / ledCurrent) * 100, 0, 200) : 0;
...
function runSelfTests() {
  const disabledInput = calculatePhototransistorSimulation({
    enabled: false,
    inputVoltage: 5,
    isolation: "Medium",
  });

  const normalInput = calculatePhototransistorSimulation({
    enabled: true,
    inputVoltage: 5,
    isolation: "Medium",
  });

  const highIsolation = calculatePhototransistorSimulation({
    enabled: true,
    inputVoltage: 5,
    isolation: "High",
  });

  const lowInput = calculatePhototransistorSimulation({
    enabled: true,
    inputVoltage: 1,
    isolation: "Low",
  });
...
const DOT_PATH_OFFSET = {
  ...
  outputPositiveToCollector: { x: 0, y: 0 },
  outputCollectorToEmitter: { x: 0, y: 0 },
  outputEmitterToLoadToNegative: { x: 0, y: 0 },
} as const satisfies Record<string, Offset>;
...
const COMPONENT = {
  ...
  phototransistor: scaleComponent(
    {
      ...BASE_COMPONENT.phototransistor,
      x:
        BASE_COMPONENT.phototransistor.x +
        COMPONENT_OFFSET.phototransistor.x,
      y:
        BASE_COMPONENT.phototransistor.y +
        COMPONENT_OFFSET.phototransistor.y,
    },
    SCALE.CIRCUIT_COMPONENT_SCALE.phototransistor,
  ),
...
const BASE_NODE = {
  ...
  collector: { x: 956, y: 224 },
  transistorBaseTop: { x: 912, y: 216 },
  transistorBaseBottom: { x: 912, y: 340 },
  transistorMid: { x: 912, y: 256 },
  transistorLow: { x: 912, y: 318 },
  emitter: { x: 958, y: 360 },

  outputTop: { x: 956, y: 112 },
  outputBottom: { x: 958, y: 472 },
...
const NODE = {
  ...
  collector: addPoint(BASE_NODE.collector, COMPONENT_OFFSET.phototransistor),
  transistorBaseTop: addPoint(
    BASE_NODE.transistorBaseTop,
    COMPONENT_OFFSET.phototransistor,
  ),
  transistorBaseBottom: addPoint(
    BASE_NODE.transistorBaseBottom,
    COMPONENT_OFFSET.phototransistor,
  ),
  transistorMid: addPoint(
    BASE_NODE.transistorMid,
    COMPONENT_OFFSET.phototransistor,
  ),
  transistorLow: addPoint(
    BASE_NODE.transistorLow,
    COMPONENT_OFFSET.phototransistor,
  ),
  emitter: addPoint(BASE_NODE.emitter, COMPONENT_OFFSET.phototransistor),

  outputTop: addPoint(BASE_NODE.outputTop, COMPONENT_OFFSET.phototransistor),
  outputBottom: addPoint(
    BASE_NODE.outputBottom,
    COMPONENT_OFFSET.phototransistor,
  ),
...
const WIRE = {
  ...
  // Input battery wires are split so no wire passes through the battery symbol.
  ...
  // Switch output to pin 1 and LED top lead.
  ...
  // Pin 1 rail down to LED anode.
  ledTopDrop: offsetPoints(
    [{ x: NODE.ledTop.x, y: NODE.pin1.y }, NODE.ledTop],
    WIRE_OFFSET.ledTopDrop,
  ),

  // LED cathode down to pin 2 level.
  ledBottomDrop: offsetPoints(
    [NODE.ledBottom, NODE.ledReturn],
    WIRE_OFFSET.ledBottomDrop,
  ),

  // FIXED: Input return wire from LED cathode / pin 2 back to battery negative.
  inputReturn: offsetPoints(
    [NODE.ledReturn, NODE.pin2, NODE.batteryBottom],
    WIRE_OFFSET.inputReturn,
  ),

  // Phototransistor collector and emitter leads.
  transistorCollectorLead: offsetPoints(
    [NODE.outputTop, NODE.collector],
    WIRE_OFFSET.transistorLeads,
  ),

  transistorEmitterLead: offsetPoints(
    [NODE.emitter, NODE.outputBottom],
    WIRE_OFFSET.transistorLeads,
  ),

  // Output side Vcc positive rail to pin 3 / collector.
  outputTop: offsetPoints(
    [NODE.outputTop, NODE.pin3, NODE.vccTop],
    WIRE_OFFSET.outputTop,
  ),

  // Output Vcc battery wires are split so no wire passes through the battery symbol.
...
/* ---------------------------------------------
   ...
   These paths describe conventional current direction only.
   Electrical current dots never cross the isolation barrier.
   Only the light arrows cross the isolation barrier.
--------------------------------------------- */

const CURRENT_PATH_POINTS = {
  // Input side: Battery + -> switch -> pin 1 -> LED anode.
  ...
  // Input side: LED cathode -> pin 2 -> battery negative.
  ...
  // Output side: Vcc + -> pin 3 -> collector.
  outputPositiveToCollector: offsetPoints(
    [
    ...
    DOT_PATH_OFFSET.outputPositiveToCollector,
  ),

  // Output side: collector -> phototransistor channel -> emitter.
  outputCollectorToEmitter: offsetPoints(
    [
      NODE.collector,
      NODE.transistorMid,
      NODE.transistorLow,
    ...
    DOT_PATH_OFFSET.outputCollectorToEmitter,
  ),

  // Output side: emitter -> pin 4 -> load -> Vcc negative.
  outputEmitterToLoadToNegative: offsetPoints(
    [
      NODE.emitter,
      NODE.outputBottom,
      NODE.pin4,
    ...
    DOT_PATH_OFFSET.outputEmitterToLoadToNegative,
  ),
} as const;
...
const CURRENT_PATH = {
  ...
  outputPositiveToCollector: pathD(
    CURRENT_PATH_POINTS.outputPositiveToCollector,
  ),
  outputCollectorToEmitter: pathD(CURRENT_PATH_POINTS.outputCollectorToEmitter),
  outputEmitterToLoadToNegative: pathD(
    CURRENT_PATH_POINTS.outputEmitterToLoadToNegative,
  ),
} as const;
...
function InputLed({ active }: { active: boolean }) {
  ...
  return (
    <g>
      ...
      <motion.g
        animate={{
          opacity: active ? [0.35, 1, 0.35] : 0.16,
        }}
        transition={{
          duration: 1.15,
          repeat: Infinity,
      ...
      >
        <line
          x1={NODE.ledTop.x + 38}
          y1={NODE.ledTop.y + 25}
          x2={NODE.collector.x - 68}
          y2={NODE.collector.y + 2}
          stroke={STYLE.light}
          strokeWidth="5"
          strokeLinecap="round"
        />
        <polygon
          points={`${NODE.collector.x - 78},${NODE.collector.y - 5} ${
            NODE.collector.x - 55
          },${NODE.collector.y} ${NODE.collector.x - 72},${
            NODE.collector.y + 13
          }`}
          fill={STYLE.light}
        />
        ...
        <line
          ...
          x2={NODE.transistorMid.x - 32}
          y2={NODE.transistorMid.y + 15}
          stroke={STYLE.light}
          strokeWidth="5"
          strokeLinecap="round"
        />
        <polygon
          points={`${NODE.transistorMid.x - 43},${NODE.transistorMid.y + 8} ${
            NODE.transistorMid.x - 20
          },${NODE.transistorMid.y + 18} ${NODE.transistorMid.x - 40},${
            NODE.transistorMid.y + 30
          }`}
          fill={STYLE.light}
        />
      </motion.g>
...
/* ---------------------------------------------
   Circuit block: phototransistor output symbol
--------------------------------------------- */
...
function PhototransistorOutput() {
  return (
    <g>
      ...
      <line
        x1={NODE.transistorBaseTop.x}
        y1={NODE.transistorBaseTop.y}
        x2={NODE.transistorBaseBottom.x}
        y2={NODE.transistorBaseBottom.y}
        stroke={STYLE.wire}
        strokeWidth="8"
        strokeLinecap="round"
      />
      ...
      <line
        x1={NODE.transistorBaseBottom.x}
        y1={NODE.transistorLow.y}
        x2={NODE.emitter.x}
        y2={NODE.emitter.y}
        stroke={STYLE.wire}
        strokeWidth="7"
      ...
      <polygon
        points={`${NODE.emitter.x - 18},${NODE.emitter.y - 26} ${
          NODE.emitter.x + 8
        },${NODE.emitter.y - 1} ${NODE.emitter.x - 25},${
          NODE.emitter.y + 4
        }`}
        fill={STYLE.wire}
      />
    ...
    <>
      ...
      {showCurrentDots && (
        <>
          ...
          <FlowDots
            path={CURRENT_PATH.outputPositiveToCollector}
            active={active}
          ...
          <FlowDots
            path={CURRENT_PATH.outputCollectorToEmitter}
            active={active}
            color={STYLE.outputCurrent}
            count={7}
            duration={1.75}
          ...
          <FlowDots
            path={CURRENT_PATH.outputEmitterToLoadToNegative}
            active={active}
            color={STYLE.outputCurrent}
            count={13}
            duration={2.45}
...
function PhototransistorCircuitSvg({
  active,
  showCurrentDots,
  showDebugTerminals,
  showFlowDebugPaths,
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Phototransistor optocoupler switching circuit schematic"
    >
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl">
      ...
      <div className="mt-5 rounded-2xl bg-blue-50 p-4 ring-1 ring-blue-100">
        <p className="text-sm font-black text-blue-800">Selected Type</p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        ...
        <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
          ...
          <p className="mt-2 text-2xl font-black text-green-600">
            {formatNumber(results.outputCurrent, 1)}mA
          </p>
        </div>
      ...
      <div className="mt-6 rounded-3xl bg-white p-4 ring-1 ring-slate-200">
        ...
        <p className="mt-2 text-sm leading-relaxed text-slate-700">
          When the input LED is ON, light activates the phototransistor. The
          output side then conducts through the external Vcc and load circuit
          while remaining electrically isolated from the input side.
        </p>
...
export default function PhototransistorCircuitControlPanel({
  initialEnabled = true,
  ...
  className,
}: PhototransistorCircuitControlPanelProps) {
  ...
  const results = useMemo(
    () =>
      calculatePhototransistorSimulation({
        enabled,
        inputVoltage,
        isolation,
      }),
  ...
  const timelineCircuitActive =
    ...
    enabled &&
    timelineStep > 0 &&
    results.ledCurrent > 1;
  const active =
    controlMode === "timeline" ? timelineCircuitActive : results.ledCurrent > 1;
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-black text-slate-900">
                LED to Optical Isolation to Phototransistor Output
              </h1>
            </div>
