...
function calculateMeasuredResistance({
  ...
  if (leadConnection === "redOpen" || leadConnection === "blackOpen") return Infinity;
...
function getDisplayText({
  ...
  if (dialMode === "continuity") {
    ...
    if (measuredResistance !== null && Number.isFinite(measuredResistance) && measuredResistance < 30) return "BEEP";
    return "OPEN";
  }
...
function getSafetyStatus({
  ...
  if (externalPower) {
    return {
      ...
      detail: "Never measure resistance on a powered circuit. Turn power off first.",
      tone: "bad",
    };
  }
  ...
  if (leadConnection === "redOpen") {
    return {
      label: "Red probe disconnected",
      detail: "The red probe is not touching the resistor, so the meter reads open loop.",
      tone: "warn",
    };
  }
  ...
  if (leadConnection === "blackOpen") {
    return {
      label: "Black probe disconnected",
      detail: "The black probe is not touching the resistor, so the meter reads open loop.",
      tone: "warn",
    };
  }
...
function getLearningFeedback({
  ...
  const dialCorrect = dialMode === "ohm";
  const leadsCorrect = leadConnection === "connected";
  const powerCorrect = !externalPower;
  const allCorrect = dialCorrect && leadsCorrect && powerCorrect;

  const messages = [
    dialCorrect
      ? "Dial mode is correct: Ω mode is selected."
      : "Dial mode is wrong: select Ω mode.",
    leadsCorrect
      ? "Probe connection is correct: both probes touch the resistor terminals."
      : "Probe connection is wrong: both probes must touch the resistor terminals.",
    powerCorrect
      ? "Safety is correct: the resistor is unpowered."
      : "Safety problem: resistance must be measured with power OFF.",
  ];
...
function DebugDot({
  ...
  return (
    <g fontFamily="Arial, Helvetica, sans-serif">
      <circle cx={x} cy={y} r={7} fill={color} stroke={COLORS.white} strokeWidth={3} />
...
function DynamicDisplay({ value }: { value: string }) {
  ...
  return (
    <g>
      <rect x={166} y={102} width={383} height={146} rx={8} fill={COLORS.white} />
      <text
        x={356}
        y={176}
        textAnchor="middle"
...
function Wire({
  ...
  const wireWidth = width * CIRCUIT_WIRE_SCALE;
  const mainColor = active ? color : COLORS.gray;
  const darkColor = active ? dark : "#4b5563";

  return (
    <g fill="none" strokeLinecap="round" strokeLinejoin="round">
      <path d={d} stroke={darkColor} strokeWidth={wireWidth + 3} opacity={active ? 0.55 : 0.25} />
      <path d={d} stroke={mainColor} strokeWidth={wireWidth} opacity={active ? 1 : 0.48} />
      <path d={d} stroke={COLORS.white} strokeWidth={1.6} opacity={active ? 0.25 : 0.12} />
    </g>
  );
}
...
function ExplanationCards({
  ...
  return (
    <div className="grid gap-2">
      <ExplanationCard
        title="Resistance"
        text={`The resistor value is set to ${formatResistance(resistorValue)}. The meter reads ${measuredResistance === null ? "ERR" : formatResistance(measuredResistance)}.`}
        tone={dialMode === "ohm" && leadConnection === "connected" && !externalPower ? "good" : "warn"}
      />

      <ExplanationCard
        title="Ω mode"
        text="To measure resistance, the dial must be on Ω mode. Using V or A mode will not give the proper resistance reading."
        tone={dialMode === "ohm" ? "good" : "warn"}
      />

      <ExplanationCard
        title="Probe connection"
        text="The black probe goes to COM and the red probe goes to V/Ω. The probes touch both resistor terminals."
        tone={leadConnection === "connected" ? "good" : "warn"}
      />

      <ExplanationCard
        title="Safety"
        text="Resistance should be measured only when the component is not powered. Powered circuits can damage the meter."
...
function ControlPanel({
  ...
  return (
    ...
    >
      <div className="mb-4">
        <h2 className="text-lg font-bold text-slate-900">Resistance Measurement</h2>
        <p className="mt-1 text-sm leading-6 text-slate-600">
          Measure a resistor with the multimeter in Ω mode. The probes go across the resistor, because apparently even tiny parts demand proper attention.
        </p>
      </div>

      <div className="space-y-3">
        <PanelSection title="Learning Mode" helper="Choose how much guidance appears on the canvas.">
          <div className="grid grid-cols-3 gap-2">
            <ToggleButton active={learningMode === "normal"} onClick={() => setLearningMode("normal")} ariaLabel="Set normal learning mode">
              Normal
            </ToggleButton>
            <ToggleButton active={learningMode === "beginner"} onClick={() => setLearningMode("beginner")} ariaLabel="Set beginner learning mode">
              Beginner
            </ToggleButton>
            <ToggleButton active={learningMode === "quiz"} onClick={() => setLearningMode("quiz")} ariaLabel="Set quiz learning mode">
              Quiz
            </ToggleButton>
          </div>
          ...
          {learningMode === "beginner" ? (
            <div className="mt-3 rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs leading-5 text-blue-800">
              In resistance mode, the meter sends a tiny test signal through the resistor and calculates its resistance.
            </div>
          ) : null}

          {learningMode === "quiz" ? (
            <div className="mt-3 rounded-xl border border-purple-200 bg-purple-50 p-3 text-xs leading-5 text-purple-800">
              Choose Ω mode, connect both probes to the resistor, and keep external power OFF. Then press Check Answer.
            </div>
          ) : null}
        </PanelSection>
        ...
        <PanelSection title="Dial mode" helper="For resistance, select Ω.">
          <div className="grid grid-cols-2 gap-2">
            <ToggleButton active={dialMode === "off"} onClick={() => setDialMode("off")} ariaLabel="Set dial to off">
              OFF
            </ToggleButton>
            <ToggleButton active={dialMode === "ohm"} onClick={() => setDialMode("ohm")} ariaLabel="Set dial to ohm mode">
              Ω
            </ToggleButton>
            ...
            <ToggleButton active={dialMode === "continuity"} onClick={() => setDialMode("continuity")} ariaLabel="Set dial to continuity mode">
              Continuity
            </ToggleButton>
            <ToggleButton active={dialMode === "diode"} onClick={() => setDialMode("diode")} ariaLabel="Set dial to diode mode">
              Diode
            </ToggleButton>
          </div>
        </PanelSection>

        <PanelSection title="Probe connection" helper="Both probes must touch the resistor terminals.">
          <div className="grid grid-cols-2 gap-2">
            ...
            <ToggleButton active={leadConnection === "shorted"} onClick={() => setLeadConnection("shorted")} ariaLabel="Short the probes">
              Shorted
            </ToggleButton>
          </div>
        </PanelSection>

        <PanelSection title="Safety" helper="Resistance is measured only on an unpowered component.">
          <div className="grid grid-cols-2 gap-2">
        ...
        <PanelSection title="Display options" helper="Show the meter test signal and debug terminal dots.">
          <div className="grid grid-cols-2 gap-2">
            <ToggleButton
        ...
        <PanelSection title="Live result" helper="The display follows the dial, probes, and safety state.">
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div className="rounded-lg bg-slate-100 p-2">
              ...
              <div className="font-bold text-slate-900">
                {measuredResistance === null ? "ERR" : formatResistance(measuredResistance)}
              </div>
            </div>
          </div>
...
function MeterPortsAndInsertedPlugs() {
  return (
    <g>
      ...
      <Port cx={PORT.voltOhm.x} cy={PORT.voltOhm.y} strokeColor={COLORS.redDark} active />
...
function LeadsAndProbes({
  ...
  return (
    <g>
      <Wire d={BLACK_CABLE_PATH} color={COLORS.black} dark="#000000" width={10} active={blackActive} />
      <Wire d={RED_CABLE_PATH} color={COLORS.red} dark={COLORS.redDark} width={10} active={redActive} />
      ...
      <g opacity={blackActive ? 1 : 0.35}>
        ...
        <path d={BLACK_PROBE_NEEDLE_PATH} fill="none" stroke={COLORS.black} strokeWidth={3} strokeLinecap="round" />
      </g>
      ...
      <g opacity={redActive ? 1 : 0.35}>
        ...
        <path d={RED_PROBE_NEEDLE_PATH} fill="none" stroke={COLORS.black} strokeWidth={3} strokeLinecap="round" />
      </g>
...
function AnimatedDot({
  ...
  return (
    <circle r={7} fill={color} opacity={0.9}>
      <animateMotion dur="2.6s" repeatCount="indefinite" path={path} begin={begin} />
    </circle>
  );
}
...
function TestSignalAnimation({
  ...
  return (
    <g>
      ...
      <path d={RED_CABLE_PATH} stroke={COLORS.redDark} strokeWidth={3} strokeDasharray="16 20" opacity={0.35} fill="none" />
      <path d={BLACK_CABLE_PATH} stroke={COLORS.black} strokeWidth={3} strokeDasharray="16 20" opacity={0.35} fill="none" />
    </g>
  );
}
...
function DebugLayer() {
  return (
    <g>
      ...
      <DebugDot x={PORT.voltOhm.x} y={PORT.voltOhm.y} label="V/Ω" color={COLORS.red} />
      <DebugDot x={NODE.blackProbeTip.x} y={NODE.blackProbeTip.y} label="BLACK TIP" color={COLORS.black} />
      <DebugDot x={NODE.redProbeTip.x} y={NODE.redProbeTip.y} label="RED TIP" color={COLORS.red} />
      <DebugDot x={NODE.resistorLeft.x} y={NODE.resistorLeft.y} label="R LEFT" color={COLORS.green} />
      <DebugDot x={NODE.resistorRight.x} y={NODE.resistorRight.y} label="R RIGHT" color={COLORS.green} />
    </g>
  );
}
...
function Callout({
  ...
  return (
    <g fontFamily="Arial, Helvetica, sans-serif">
      ...
      <rect x={boxX} y={boxY} width={width} height={36} rx={10} fill={COLORS.white} stroke={color} strokeWidth={2.5} />
      <text
        x={boxX + width / 2}
        y={boxY + 19}
        textAnchor="middle"
...
function BeginnerAnnotations() {
  return (
    <g>
      <Callout x={354} y={323} boxX={170} boxY={275} text="Dial on Ω mode" width={190} color={COLORS.blue} />
      <Callout x={PORT.com.x} y={PORT.com.y} boxX={130} boxY={850} text="Black lead to COM" width={210} color={COLORS.black} />
      <Callout x={PORT.voltOhm.x} y={PORT.voltOhm.y} boxX={400} boxY={850} text="Red lead to V/Ω" width={210} color={COLORS.red} />
      <Callout x={NODE.blackProbeTip.x} y={NODE.blackProbeTip.y} boxX={720} boxY={300} text="Probe touches one end" width={240} color={COLORS.black} />
      <Callout x={NODE.redProbeTip.x} y={NODE.redProbeTip.y} boxX={1110} boxY={300} text="Probe touches other end" width={260} color={COLORS.red} />
      <Callout x={NODE.resistorBodyCenter.x} y={NODE.resistorBodyCenter.y} boxX={965} boxY={470} text="Resistor under test" width={230} color={COLORS.amber} />
      <Callout x={355} y={180} boxX={620} boxY={135} text="Display shows resistance" width={250} color={COLORS.green} />
    </g>
  );
}
    ...
    >
      ...
      >
        ...
        <g transform={`scale(${CIRCUIT_CANVAS_SCALE})`}>
          <MeterBody displayText={displayText} dialMode={dialMode} safetyStatus={safetyStatus} />
          <MeterPortsAndInsertedPlugs />
          <TestSignalAnimation active={testSignalActive} show={showTestSignal} />
          <LeadsAndProbes leadConnection={leadConnection} />
          <ResistorFigure resistorValue={resistorValue} />

          {safetyStatus.tone === "bad" ? (
            <g>
              <rect x={690} y={720} width={465} height={72} rx={14} fill="#fee2e2" stroke={COLORS.red} strokeWidth={4} />
              <Label x={922} y={745} size={23} fill={COLORS.red}>
                Never measure resistance on powered circuit
              </Label>
              <Label x={922} y={772} size={17} fill={COLORS.red}>
                Turn external power OFF before measuring Ω
              </Label>
            </g>
          ) : null}

          {dialMode !== "ohm" && dialMode !== "off" && safetyStatus.tone !== "bad" ? (
            <g>
              <rect x={665} y={720} width={360} height={66} rx={14} fill="#fef3c7" stroke={COLORS.amber} strokeWidth={4} />
              <Label x={845} y={745} size={22} fill="#92400e">
                Wrong dial mode
              </Label>
              <Label x={845} y={770} size={17} fill="#92400e">
...
export default function MeasuringResistanceSketch({
  ...
  const [resistorValue, setResistorValueState] = useState(220);
  const [dialMode, setDialModeState] = useState<DialMode>("ohm");
  ...
  function setResistorValue(value: number) {
    setResistorValueState(value);
    markQuizDirty();
  }
  ...
  function reset() {
    setResistorValueState(220);
    setDialModeState("ohm");
