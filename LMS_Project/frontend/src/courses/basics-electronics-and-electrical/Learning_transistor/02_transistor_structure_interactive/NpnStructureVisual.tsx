...
const NODE = {
  ...
  topLeft: { x: 60, y: 118 },
  vceMinus: { x: 385, y: 118 },
  vcePlus: { x: 440, y: 118 },
  topRight: { x: 865, y: 118 },
  rightDown: { x: 865, y: 310 },

  emitterDown: { x: 60, y: 535 },
  baseSupplyMinus: { x: 150, y: 535 },
  baseSupplyPlus: { x: 190, y: 535 },
...
const WIRE = {
  ...
  collectorSupplyLeft: [NODE.topLeft, NODE.vceMinus],

  // Routed around lamp bulb area, not through lamp center.
  ...
  emitterGroundReturn: [
    { x: 60, y: 310 },
    NODE.emitterDown,
    NODE.baseSupplyMinus,
  ],

  baseBiasLeft: [NODE.baseSupplyPlus, NODE.switchA],

  // Routed around resistor body.
  baseBiasRight: [
    NODE.switchB,
  ...
  // Routed around resistor body, then into top terminal.
...
function TransistorLayers({
  ...
  const ebBias = clamp(baseVoltage / 1.2, 0, 1);
  ...
  return (
    <g>
      ...
      <rect
        ...
        strokeWidth="2"
      />
      <rect x="210" y="225" width="168" height="170" fill="url(#nEmitter)" />
      <rect x="378" y="225" width="56" height="170" fill="url(#pBase)" />
      <rect x="434" y="225" width="336" height="170" fill="url(#nCollector)" />
      ...
      <text x="290" y="205" fill="#0b56c5" fontSize="24" fontWeight="900">
        N+
      </text>
      <text x="395" y="205" fill="#dc2626" fontSize="24" fontWeight="900">
        P
      </text>
      <text x="590" y="205" fill="#0b56c5" fontSize="24" fontWeight="900">
        N
      </text>
      ...
      <text x="260" y="418" fill="#0b56c5" fontSize="16" fontWeight="900">
        Emitter (N+)
      </text>
      <text x="385" y="418" fill="#dc2626" fontSize="16" fontWeight="900">
        Base (P)
      </text>
      <text x="560" y="418" fill="#0b56c5" fontSize="16" fontWeight="900">
        Collector (N)
      </text>

      {showPhysics && (
        <>
          <text x="248" y="330" fill="#0b56c5" fontSize="13" fontWeight="900">
            {/* Emitter injects electrons */}
          </text>
          <text x="350" y="338" fill="#7f1d1d" fontSize="12" fontWeight="900">
            {/* VBE ↑ → EB depletion ↓ */}
          </text>
          <text x="350" y="353" fill="#7f1d1d" fontSize="12" fontWeight="900">
            {/* Thin base minimizes recombination */}
          </text>
          <text x="553" y="330" fill="#0b56c5" fontSize="13" fontWeight="900">
            {/* VCE ↑ → CB depletion ↑ */}
          </text>
        </>
      )}
...
function ElectronFlow({
  ...
  const mainElectronCount = Math.round(10 + speedFactor * 10);
  ...
  return (
    <g>
      ...
      {Array.from({ length: mainElectronCount }).map((_, index) => (
        <motion.circle
          key={`main-electron-${index}`}
          r="5"
          fill={STYLE.electron}
...
function BaseCurrentFlow({
  ...
  const path = `M${NODE.rbBottom.x} ${NODE.rbBottom.y} L${NODE.rbBypassX.x} ${NODE.rbBottom.y} L${NODE.rbBypassX.x} ${NODE.rbTop.y} L${NODE.rbTop.x} ${NODE.rbTop.y} C${NODE.rbTop.x} 410 ${NODE.rbTop.x - 17} 398 ${NODE.b.x} ${NODE.b.y}`;
  ...
  return (
    <g>
      <path
        ...
        strokeDasharray="6 7"
        fill="none"
        markerEnd="url(#redArrowUp)"
      />

      {Array.from({ length: particleCount }).map((_, index) => (
        <motion.circle
          key={`base-current-${index}`}
          r="4"
          fill={STYLE.hole}
          initial={{ x: NODE.rbBottom.x, y: NODE.rbBottom.y, opacity: 0 }}
          animate={{
            x: [
              NODE.rbBottom.x,
              NODE.rbBypassX.x,
...
function CurrentArrows({ active, show }: { active: boolean; show: boolean }) {
  ...
  const ibPath = `M${NODE.rbBottom.x} ${NODE.rbBottom.y} L${NODE.rbBypassX.x} ${NODE.rbBottom.y} L${NODE.rbBypassX.x} ${NODE.rbTop.y} L${NODE.rbTop.x} ${NODE.rbTop.y} C${NODE.rbTop.x} 410 ${NODE.rbTop.x - 17} 398 ${NODE.b.x} ${NODE.b.y}`;

  return (
    <g opacity={active ? 1 : 0.25}>
      <path
        d="M770 310 C690 300 570 292 434 310"
        stroke={STYLE.current}
        strokeWidth="3"
        fill="none"
        strokeDasharray="8 7"
        markerEnd="url(#purpleArrowLeft)"
      />
      <text x="600" y="285" fill={STYLE.current} fontSize="13" fontWeight="900">
        {/* IC: Collector → Device */}
      </text>
      ...
      <path
        d="M390 325 C315 320 255 315 210 310"
        stroke={STYLE.current}
        strokeWidth="3"
        fill="none"
        strokeDasharray="8 7"
        markerEnd="url(#purpleArrowLeft)"
      />
      <text x="250" y="345" fill={STYLE.current} fontSize="13" fontWeight="900">
        {/* IE: Device → Emitter */}
      </text>
    </g>
  );
...
export default function NpnStructureVisual({
  active,
  dopingLevel,
  collectorVoltage,
  baseVoltage,
  ...
  showCurrent,
  showElectronFlow,
}: NpnStructureVisualProps) {
  const calc = useMemo(() => {
    const betaLimit = clamp(dopingLevel * 2, 20, 200);
    const ibA = active ? Math.max((baseVoltage - 0.7) / baseResistance, 0) : 0;
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-black text-slate-900">
            NPN Transistor Structure Simulator
          </h2>
          <p className="text-sm text-slate-600">
            Wire routing fixed: wires avoid resistor and lamp body.
          </p>
        </div>
        ...
        <svg viewBox={VIEW_BOX} className="h-auto w-full">
          <defs>
            ...
            <marker
              ...
              orient="auto"
            >
              <path d="M0 10 L5 0 L10 10 Z" fill={STYLE.hole} />
            </marker>
            <marker
              id="purpleArrowLeft"
              markerWidth="10"
              ...
              orient="auto"
            >
              <path d="M10 0 L0 5 L10 10 Z" fill={STYLE.current} />
            </marker>
            <marker
              id="purpleArrowToBase"
              markerWidth="10"
              markerHeight="10"
              refX="9"
              refY="5"
            ...
            <linearGradient id="nEmitter" x1="0" x2="1">
              <stop offset="0%" stopColor="#dbeafe" />
              <stop offset="100%" stopColor="#eff6ff" />
            </linearGradient>
            <linearGradient id="pBase" x1="0" x2="1">
              <stop offset="0%" stopColor="#fee2e2" />
              <stop offset="100%" stopColor="#fff1f2" />
            </linearGradient>
            <linearGradient id="nCollector" x1="0" x2="1">
          ...
          <BatterySymbol
            x={400}
            y={118}
            label={`VCE = +${format(collectorVoltage, 1)}V`}
            leftSign="-"
            rightSign="+"
          />
          <BatterySymbol
