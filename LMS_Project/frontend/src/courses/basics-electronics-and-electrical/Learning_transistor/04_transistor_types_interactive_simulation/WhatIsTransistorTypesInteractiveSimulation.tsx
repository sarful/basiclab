...
import LibraryTransistorStage from "./LibraryTransistorStage";
import { buildCurrentFlowModel } from "./currentFlow";
...
export default function WhatIsTransistorTypesInteractiveSimulation() {
  ...
  const current = useMemo(() => {
    const base =
      family === "BJT"
        ? signal * gain * 0.002
        : fetType === "JFET"
          ? signal * (0.045 + gain * 0.00018)
          : Math.max(0, signal - 18) * (0.18 + gain * 0.0016);
    return clamp(base, 0, 100);
  }, [family, fetType, signal, gain]);
  const activeThreshold =
    family === "BJT" ? 1 : fetType === "JFET" ? 4 : 8;
  const active = current >= activeThreshold;
  ...
  const mainCarrier = family === "BJT"
    ? bjtType === "NPN"
      ? "Majority carriers: electrons | Conventional current: C -> E"
      : "Majority carriers: holes | Conventional current: E -> C"
    : fetChannel === "N-Channel"
      ? "Majority carriers: electrons | Conventional current: D -> S"
      : "Majority carriers: holes | Conventional current: S -> D";
  const currentFlowModel = useMemo(
    () =>
      buildCurrentFlowModel({
        family,
        bjtType,
        fetType,
        fetChannel,
    ...
    [active, bjtType, current, family, fetChannel, fetType, gain, signal],
  );
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        <div className="grid gap-5 xl:grid-cols-3">
          ...
          <div className="space-y-5 xl:col-span-2">
              ...
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    Library symbol view for the selected transistor type.
                  </p>
                </div>
                <div
            ...
            <CurrentFlowPanel model={currentFlowModel} />
          </div>
        </div>
