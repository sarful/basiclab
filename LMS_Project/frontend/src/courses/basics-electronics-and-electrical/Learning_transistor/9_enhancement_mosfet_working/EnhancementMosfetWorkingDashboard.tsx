...
import { getStateColor, getThermalColor } from "./enhancementMosfetLogic";
...
type EnhancementMosfetWorkingDashboardProps = {
  gateVoltage: number;
...
export default function EnhancementMosfetWorkingDashboard({
  gateVoltage,
  thresholdVoltage,
  drainVoltage,
  saturationVoltage,
  ...
  flowMode,
  learningMode,
}: EnhancementMosfetWorkingDashboardProps) {
  ...
  const cards = [
    ...
    { title: "Threshold Voltage", value: thresholdVoltage.toFixed(1), unit: "V", color: "#ef4444", gaugeValue: thresholdVoltage, gaugeMax: 5 },
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-emerald-300">
              Enhancement MOSFET Working Dashboard
            </p>
          ...
          <div className="flex flex-wrap gap-2 text-xs font-bold uppercase tracking-[0.08em]">
            <span className="rounded-full bg-white/10 px-3 py-1.5">SCADA {regionState === "OFF" ? "Idle" : "Live"}</span>
