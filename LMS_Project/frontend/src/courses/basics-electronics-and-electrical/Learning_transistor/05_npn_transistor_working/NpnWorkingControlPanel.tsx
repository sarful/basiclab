...
type NpnWorkingControlPanelProps = {
  controls: NpnWorkingControls;
  onChange: (patch: Partial<NpnWorkingControls>) => void;
  onReset: () => void;
};
...
export default function NpnWorkingControlPanel({
  controls,
  onChange,
  onReset,
}: NpnWorkingControlPanelProps) {
  return (
    <div className="space-y-4">
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl">
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <button
            type="button"
            onClick={() => onChange({ switchClosed: !controls.switchClosed, presetMode: "manual" })}
      ...
      <NumericSliderRow
        ...
        step={0.1}
        suffix="V"
        onChange={(value) => onChange({ batteryVoltage: value, presetMode: "manual" })}
      />

      <NumericSliderRow
        label="RB"
        value={controls.rbOhms}
        min={1000}
        max={47000}
        step={100}
        suffix="Ohm"
        onChange={(value) => onChange({ rbOhms: value, presetMode: "manual" })}
      />

      <NumericSliderRow
        label="RPD"
        value={controls.rpdOhms}
        min={1000}
        max={100000}
        step={500}
        suffix="Ohm"
        onChange={(value) => onChange({ rpdOhms: value, presetMode: "manual" })}
      />
      ...
      <NumericSliderRow
        ...
        step={50}
        suffix="Ohm"
        onChange={(value) => onChange({ rLedOhms: value, presetMode: "manual" })}
      />
