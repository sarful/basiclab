...
const INFO: Record<ComponentKey, { name: string; purpose: string }> = {
  ...
  field: {
    ...
    purpose: "Pulls the relay contact closed.",
  },
  contact: {
    name: "SPST Normally Open Contact",
    purpose: "Closes the AC lamp circuit when the relay coil is energized.",
  },
  lamp: {
    name: "Lamp",
    purpose: "Turns ON when the AC circuit becomes complete.",
  },
  ac: {
    name: "AC Source",
...
const STEP_TEXT: Record<SimulationState, string> = {
  Idle: "Relay is idle. NO contact is open. Lamp is OFF.",
  ...
  ContactClosing: "The NO contact closes.",
  LampOn: "AC circuit is complete. Lamp turns ON.",
  Resetting: "Button released. Relay returns to normal open state.",
};
...
function RelayContact({
  ...
  const finalClosed = closed || stuck;
    ...
    <g onClick={onClick} className="cursor-pointer">
      ...
      >
        {bouncing && (
          <animate
            attributeName="x2"
            values="730;768;748;768;758;768"
            dur="0.35s"
            repeatCount="1"
          />
        )}
...
function TimelinePanel({
  ...
  const items: { state: SimulationState; title: string }[] = [
    ...
    { state: "ContactClosing", title: "Contact" },
    { state: "LampOn", title: "Lamp" },
  ];
  ...
  return (
    <aside className="space-y-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm ring-1 ring-slate-100">
      <div className="rounded-2xl bg-slate-950 p-4 text-white">
        ...
        <h3 className="mt-1 text-xl font-black">SPST NO Relay</h3>
      ...
      <div className="rounded-2xl bg-slate-50 p-3 text-sm font-semibold text-slate-700 ring-1 ring-slate-200">
        {controlMode === "onOff"
          ? `ON/OFF Mode: Start keeps the relay energized until Stop is pressed. Current status: ${
              manualOn ? "ON" : "OFF"
            }.`
          : "Timeline Mode: Start plays the relay operation step by step."}
      </div>
  ...
  return (
    <div className="rounded-xl border bg-white p-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div className="rounded-lg bg-slate-100 p-3">
          Continuity: <b>{continuity ? "Closed" : "Open"}</b>
        </div>
...
function TruthTable({
  ...
  return (
    <div className="rounded-xl border bg-white p-4">
      <h3 className="mb-3 font-bold text-slate-800">Truth Table</h3>
  ...
  return (
    <div className="w-full overflow-x-auto rounded-xl border bg-white">
      <svg
        ...
        role="img"
        aria-label="Interactive relay simulator"
...
export default function InteractiveRelaySimulator() {
  ...
  const [selected, setSelected] = useState<ComponentKey>("coil");
  ...
  const contactClosed =
    ...
    (controlMode === "onOff"
      ? fieldVisible
      : fieldVisible && hasReached(state, "ContactClosing"));

  const lampOn =
    acAvailable &&
    contactClosed &&
    !faults.burnedLamp &&
    (controlMode === "onOff" ? manualOn : hasReached(state, "LampOn"));
  ...
  const failureReason = useMemo(() => {
    if (faults.noDcSupply) return "No DC supply: relay coil cannot energize.";
    if (faults.brokenButton)
      return "Broken button: DC control path cannot close.";
    if (faults.openCoil)
      return "Open coil: no current can flow through relay coil.";
    if (faults.shortedCoil)
      return "Shorted coil: excessive current may overheat relay.";
    if (voltage < pickupVoltage) return "Voltage is below pickup voltage.";
    if (faults.noAcSupply) return "No AC supply: lamp has no power.";
    if (faults.burnedLamp)
      return "Burned lamp: circuit is complete but lamp cannot glow.";
    return "No active failure.";
  }, [faults, voltage, pickupVoltage]);
  ...
  useEffect(() => {
    if (state === "ContactClosing" && bounceTime > 0) {
      setBouncing(true);
  ...
  return (
    <div className="w-full space-y-4 bg-white p-4">
      <div className="rounded-xl border bg-gradient-to-r from-slate-900 to-slate-700 p-5 text-white">
        <h1 className="text-2xl font-bold">Interactive Relay Simulator</h1>
        <p className="text-sm text-slate-200">
          ON/OFF Mode: Start keeps relay energized. Timeline Mode: Start plays
