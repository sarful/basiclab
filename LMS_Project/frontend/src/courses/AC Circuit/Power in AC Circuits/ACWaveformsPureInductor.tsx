export default function ACWaveformsPureInductor() {
  const width = 1000;
  const height = 620;
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-white p-6">
      <div className="w-full max-w-6xl bg-white">
        ...
        >
          <title id="title">AC Waveforms for a Pure Inductor</title>
          <desc id="desc">
            AC voltage, current, and power waveforms for a pure inductor.
            Current lags voltage by 90 degrees, and instantaneous power
          ...
          {/* Power region signs */}
          <text x={mapX(0.38 * Math.PI)} y={mapY(0.82, powerAmp)} style={textStyle}>
            +
          </text>
          <text x={mapX(1.38 * Math.PI)} y={mapY(0.82, powerAmp)} style={textStyle}>
            +
          </text>

          <text x={mapX(0.88 * Math.PI)} y={mapY(-0.72, powerAmp)} style={textStyle}>
            −
          </text>
          <text x={mapX(1.88 * Math.PI)} y={mapY(-0.72, powerAmp)} style={textStyle}>
            −
          </text>

          {/* Labels and callouts */}
          <text x={mapX(0.25 * Math.PI)} y={mapY(0.9, waveAmp) - 10} style={{ ...smallTextStyle, fill: blue }}>
            v
          </text>

          <text x={mapX(0.52 * Math.PI)} y={mapY(0.58, currentAmp) - 12} style={{ ...smallTextStyle, fill: red }}>
            i
          </text>

          <text x={mapX(0.50 * Math.PI) + 160} y={mapY(0.95, powerAmp) - 20} style={smallTextStyle}>
            Power
          </text>
          <text x={mapX(0.50 * Math.PI) + 150} y={mapY(0.95, powerAmp) + 8} style={smallTextStyle}>
            Consumed
          </text>

          <line
            x1={mapX(0.50 * Math.PI) + 140}
            y1={mapY(0.88, powerAmp)}
            x2={mapX(0.32 * Math.PI)}
            y2={mapY(0.78, powerAmp)}
            stroke={dark}
            strokeWidth="2"
            markerEnd="url(#arrow)"
          />

          <text x={mapX(0.73 * Math.PI)} y={mapY(-0.95, powerAmp) + 40} style={smallTextStyle}>
            Power
          </text>
          <text x={mapX(0.73 * Math.PI)} y={mapY(-0.95, powerAmp) + 68} style={smallTextStyle}>
            Returned
          </text>

          <line
            x1={mapX(0.73 * Math.PI) + 75}
            y1={mapY(-0.82, powerAmp) + 48}
            x2={mapX(0.84 * Math.PI)}
          ...
          {/* 90 degree phase lag annotation */}
          <line
            x1={plotLeft + 20}
            y1={axisY + 42}
            x2={mapX(pi2) - 12}
            ...
            markerEnd="url(#arrow)"
          />
          <text x={plotLeft + 72} y={axisY + 70} style={tinyTextStyle}>
