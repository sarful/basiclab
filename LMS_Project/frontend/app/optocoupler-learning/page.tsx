        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {firstLesson ? (
            <Link
              ...
              className="mt-5 inline-flex rounded-2xl bg-violet-700 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-violet-800"
            >
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {track.lessons.map((lesson) => (
              <Link
                key={lesson.id}
                href={lesson.href}
                className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-slate-950 transition hover:border-violet-200 hover:bg-white hover:shadow-sm"
              >
