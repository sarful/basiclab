...
import { measurementPracticalLessons } from "./lessonData";
...
export default function MeasurementPracticalCourseNav({
  ...
  const { currentLesson, previousLesson, nextLesson, totalLessons } = useMemo(() => {
    const currentIndex = measurementPracticalLessons.findIndex(
      (lesson) => lesson.id === currentLessonId,
    );

    return {
      currentLesson: measurementPracticalLessons[currentIndex] ?? null,
      previousLesson: currentIndex > 0 ? measurementPracticalLessons[currentIndex - 1] : null,
      nextLesson:
        currentIndex >= 0 && currentIndex < measurementPracticalLessons.length - 1
          ? measurementPracticalLessons[currentIndex + 1]
          : null,
      totalLessons: measurementPracticalLessons.length,
    };
  }, [currentLessonId]);
    ...
    >
      ...
      >
        <div>
          ...
          >
            Move step by step through the practical measurement course and keep track
            of where you are.
          </p>
        </div>
      ...
      {showLessons ? (
        ...
        >
          {measurementPracticalLessons.map((lesson) => {
