...
type DolComponentKey = "mcb" | "ol" | "off" | "on" | "hold" | "k1" | "indicator";
type DolFaultScenario = "none" | "overload" | "mcb-off" | "holding-fail" | "start-fail";
type DolTimelineEvent = { id: number; time: string; title: string; detail: string };
...
const GUIDED_LESSONS = [
  ...
  },
  {
    ...
    check: (snapshot: DolLessonSnapshot) =>
      snapshot.startObserved || snapshot.startupInProgress || snapshot.motorRunning,
  },
  {
    ...
    check: (snapshot: DolLessonSnapshot) =>
      snapshot.pickupObserved || snapshot.startupInProgress || snapshot.motorRunning,
  },
  {
...
> = {
  ...
  hold: {
    label: "HOLD",
    title: "K1 Auxiliary NO Holding Contact",
...
export default function DOLStarterProjectWorkspace() {
  ...
  const [focusedComponent, setFocusedComponent] = useState<DolComponentKey>("mcb");
  ...
  const [canvasViewMode, setCanvasViewMode] = useState<DolCanvasViewMode>("fit");
  ...
  const holdingClosed = controlSupplyLive && motorRunning && !holdingContactFailed;
  const startCommandActive = controlSupplyLive && startPressed && !startButtonFailed;
  const coilEnergized = controlSupplyLive && (startCommandActive || holdingClosed);
  ...
  const mcbClosedObserved = eventTimeline.some((event) => event.title === "MCB Closed");
  const startObserved = eventTimeline.some((event) => event.title === "START Pressed");
  const pickupObserved = eventTimeline.some((event) => event.title === "K1 Pickup");
  const holdingObserved = eventTimeline.some(
    (event) => event.title === "Holding Contact Closed",
  );
  const stopObserved = eventTimeline.some((event) => event.title === "STOP Pressed");
  ...
  const showStagedFlow =
    trainingMode === "guided" || replayState !== "idle" || startupInProgress || startPressed;
  ...
  const controlPowerCorrelationTitle = !mcbOn
    ? "Control circuit is isolated."
    : overloadTripped
      ? "Protection has interrupted both control and power action."
      : motorRunning
        ? "Control circuit is driving the power circuit."
        : startupInProgress
          ? "Control circuit is pulling in the power circuit."
          : "Power circuit is waiting for a control command.";
  const controlPowerCorrelationBody = !mcbOn
    ? "MCB is open, so K1 coil cannot energize and the power contacts stay open."
    : overloadTripped
      ? "O/L NC opened the control rung, K1 dropped out, and the power contacts released the motor."
      : motorRunning
        ? "K1 coil is energized through the holding path, so the 3-phase power contacts remain closed and the motor keeps running."
        : startupInProgress
          ? "START button is energizing K1. Once K1 picks up, the main power contacts begin feeding the motor."
          : "Breaker is healthy, but K1 is still open. Press START to close the control rung and then the power contacts.";
  ...
  const currentBand: "safe" | "warning" | "trip" = overloadTripped
    ...
    : motorCurrent >= currentLimit
      ? "trip"
      : motorCurrent >= currentLimit * 0.85 || motorCurrent >= ratedCurrent * 1.15
        ? "warning"
        : "safe";
  ...
  const componentStateBadges: DolComponentStateBadge[] = [
    ...
    },
    {
      key: "hold",
      label: "K1 Hold",
      state: holdingClosed ? "Closed" : holdingContactFailed ? "Fault Open" : "Open",
      tone: holdingContactFailed ? "fault" : holdingClosed ? "active" : "neutral",
    },
    {
      key: "k1",
      label: "K1 Coil",
  ...
  const handleStart = () => {
    if (overloadTripped || !mcbOn) return;
  ...
  const handleFaultScenarioChange = (scenario: DolFaultScenario) => {
    ...
    if (scenario === "overload") {
      ...
      tripMotor({
        kind: "overload",
        current: Math.max(resolveRunningCurrent(motorHorsepower, loadPercent), currentLimit + 1),
        limit: currentLimit,
      });
      return;
    }
    ...
    setTripState({ kind: "none" });
    pushTimelineEvent(
      scenario === "start-fail" ? "Scenario: START Failure" : "Scenario: Holding Failure",
      scenario === "start-fail"
  ...
  const handleLessonNext = () => {
    ...
    setLessonIndex((current) => Math.min(current + 1, GUIDED_LESSONS.length - 1));
  };
  ...
  const sharedFullPanel = (
    <ControlPanel
      ...
      componentHelpOptions={Object.entries(COMPONENT_HELP).map(([key, value]) => ({
        key,
        label: value.label,
      }))}
          ...
          canvasContent: (
            <div className="project-workspace-logic-copy">
              <p>
                ...
                beginner-friendly DOL starter learning workspace. It shows both the
                <strong> control circuit</strong> and the
                <strong> power circuit</strong> of a 3-phase motor starter, so a
                learner can understand how the logic command and the actual motor
                power path work together.
              </p>
              <p>
                <strong>What is a DOL starter?</strong> DOL means
                <strong> Direct-On-Line starter</strong>. In this method, the motor
                is connected directly to the 3-phase line through a breaker,
                contactor, and overload relay. It is one of the simplest and most
                common motor starting methods in industrial electrical systems.
              </p>
              <p>
                ...
                actually works in sequence. This project helps solve that by showing:
              </p>
              ...
              <p>
                ...
                <strong> Start</strong>, the coil energizes, the contactor closes,
                and power goes to the motor. When you press
                <strong> Stop</strong>, or if overload trips, the contactor opens
                and the motor stops.
              </p>
              <p>
                <strong>Analogy:</strong> imagine a water pump line with a master
                valve, a remote-controlled valve, and a safety cut-off. The breaker
                is the master valve, the contactor is the remote-controlled valve,
                and the overload relay is the safety cut-off that trips if the motor
                is under too much stress.
              </p>
              <p>
                <strong>Required components and symbols from this project library:</strong>
              </p>
              <div className="project-workspace-symbol-grid">
                <TheorySymbolCard title="MCB / MCCB" symbol={<MCBControl2P on label="" textSize={4} wireStroke={1.2} />} />
                <TheorySymbolCard
                  title="Contactor Coil K1"
                  symbol={<ContactorCoil energized={false} label="K1" wireStroke={1.2} textSize={4} />}
                />
                <TheorySymbolCard
                  title="Power Contacts 3P"
                  symbol={<ContactorPowerContacts3P closed label="K1" standalone />}
                />
                <TheorySymbolCard
                  title="Auxiliary NO"
                  symbol={<AuxiliaryContactNO closed={false} label="" textSize={4} wireStroke={1.2} />}
                />
                <TheorySymbolCard title="Thermal Overload 3P" symbol={<ThermalOverloadRelay3P tripped={false} label="" />} />
                <TheorySymbolCard
                  title="O/L NC 95-96"
                  symbol={
                    <ThermalOverloadNC
                ...
                />
                <TheorySymbolCard
                  title="Stop Push Button"
                  symbol={<PushButtonNC pressed={false} label="" textSize={4} wireStroke={1.2} />}
                />
                <TheorySymbolCard
                  title="Start Push Button"
                  symbol={<PushButtonNO pressed={false} label="" textSize={4} wireStroke={1.2} />}
                />
                <TheorySymbolCard title="Pilot Light" symbol={<PilotLight on={false} label="" textSize={4} wireStroke={1.2} />} />
                <TheorySymbolCard title="AC Motor 3P" symbol={<ACMotor3P label="" />} />
              </div>
              <ul className="project-workspace-logic-list">
                <li><strong>MCB / MCCB:</strong> isolates the line supply before the control or power circuit starts.</li>
                <li><strong>Contactor Coil K1:</strong> energizes the contactor and closes the main power contacts.</li>
                <li><strong>Contactor Power Contacts 3P:</strong> connect 3-phase power to the motor.</li>
                <li><strong>Auxiliary Contact NO:</strong> creates the holding or seal-in path after the Start button is released.</li>
                <li><strong>Thermal Overload Relay 3P:</strong> protects the motor on the power side.</li>
                <li><strong>Thermal Overload NC 95-96:</strong> opens the control circuit when overload trips.</li>
                <li><strong>Push Button NC:</strong> works as the Stop button.</li>
                <li><strong>Push Button NO:</strong> works as the Start button.</li>
                <li><strong>Pilot Light:</strong> shows running or energized status.</li>
                <li><strong>AC Motor 3P:</strong> the final load of the DOL starter system.</li>
              </ul>
              <p><strong>Basic operation sequence:</strong></p>
              <ul className="project-workspace-logic-list">
                ...
                <li>The motor keeps running until Stop is pressed or overload trips.</li>
              </ul>
              <p>
                <strong>Why overload protection is important:</strong> a motor can
                draw too much current because of extra load, mechanical jam, low
                speed under stress, or incorrect settings. The overload relay helps
                protect the motor winding from overheating and damage.
              </p>
              <p>
                <strong>Limitations of a DOL starter:</strong> DOL is simple and
                low-cost, but it is not suitable for every motor application.
                Because the motor is connected directly to the full line voltage, the
                starting current can be very high.
              </p>
              <ul className="project-workspace-logic-list">
                <li>high starting current can cause voltage drop in the supply line</li>
                <li>large motors may create mechanical shock at startup</li>
                <li>there is no soft acceleration or speed control</li>
                <li>not ideal for heavy-duty loads that need smooth starting</li>
                <li>for bigger motors, methods like star-delta or soft starter may be better</li>
              </ul>
              <p>
                <strong>Beginner study tip:</strong> first understand the control
                circuit because it tells the system <em>when to run</em>. Then study
                the power circuit because it shows <em>how the motor actually receives power</em>.
              </p>
            </div>
          ),
          fullWidth: true,
