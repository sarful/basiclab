...
const phaseActive = ["#dc2626", "#111827", "#64748b"];
const phaseMuted = ["#fca5a5", "#cbd5e1", "#d8e1eb"];
...
function phaseColor(index: number, state: WireState) {
  ...
  if (state === "inactive") return phaseMuted[index];
  return phaseActive[index];
}
...
export default function DOLStarterPowerDiagram({
  ...
  const {
    ...
    contactor: {
      ...
      leftLabelX: contactorLeftLabelX,
      rightLabelX: contactorRightLabelX,
      labelY: contactorLabelY,
    },
    overload: {
      y: overloadY,
      leftLabelX: overloadLeftLabelX,
  ...
  const loadState: WireState = overloadTripped
    ...
    : motorRunning
      ? "active"
      : "inactive";
  const contactorState: WireState = overloadTripped
    ? "fault"
    : motorRunning
      ? "active"
      : mcbOn
  ...
  const speedValue = Math.round(
    motorRunning ? motorRpm * Math.max(0.45, 1 - loadPercent / 220) : 0,
  );
        ...
        >
          ...
          >
            <defs>
              ...
              <radialGradient id="motor-body-gradient" cx="34%" cy="28%" r="78%">
                <stop offset="0%" stopColor="#ffffff" />
                <stop offset="58%" stopColor="#f2f6fb" />
                <stop offset="100%" stopColor="#d9e1ec" />
              </radialGradient>
              <linearGradient id="motor-rim-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#f8fafc" />
                <stop offset="35%" stopColor="#cbd5e1" />
                <stop offset="72%" stopColor="#94a3b8" />
                <stop offset="100%" stopColor="#e2e8f0" />
              </linearGradient>
              <radialGradient id="motor-core-gradient" cx="30%" cy="26%" r="74%">
                <stop offset="0%" stopColor="#f8fafc" />
                <stop offset="100%" stopColor="#cbd5e1" />
              </radialGradient>
              <filter id="motor-shadow" x="-60%" y="-60%" width="220%" height="220%">
                <feDropShadow dx="0" dy="4" stdDeviation="5" floodColor="#0f172a" floodOpacity="0.14" />
              </filter>
              <filter id="motor-running-glow" x="-100%" y="-100%" width="300%" height="300%">
            ...
            {/* Section 1: Incoming 3-phase supply + MCCB feeder */}
            {[0, 1, 2].map((index) => (
              <g key={`source-${index}`}>
                {showLabels ? (
                  <text
            ...
            {/* Section 1: Library circuit breaker symbol for the incoming 3-phase branch. */}
            <CircuitBreaker3P
              x={breakerRenderX}
              y={breakerRenderY}
              scale={breakerScale}
            ...
            {/* Section 1: Vertical continuation from the breaker output down to K1. */}
            {phaseXs.map((x, index) => (
              <PhaseLine
            ...
            {/* Section 2: Library K1 main power contacts. */}
            ...
            {/* Section 2: Vertical phase continuation from K1 down to the overload relay. */}
            {phaseXs.map((x, index) => (
              <PhaseLine
            ...
            {/* Section 3: Library thermal overload relay. */}
            ...
            {/* Section 3: Output from overload relay to the motor terminals. */}
            {phaseXs.map((x, index) => (
              <PhaseLine
            ...
            {/* Section 4: Library 3-phase motor symbol with simulation-style motion state. */}
            <g>
              <circle
                cx={motorCenterX}
              ...
              <circle
                ...
                filter={visualMotorActive ? "url(#motor-running-glow)" : undefined}
              />
              <g opacity="0.34">
                <ACMotor3P
                  x={motorSymbolX}
              ...
              <g opacity={visualMotorActive ? 1 : 0.45}>
                ...
                <path
                  d={`M ${motorCenterX - 16} ${motorVisualCenterY - 2} Q ${motorCenterX - 2} ${motorVisualCenterY - 18} ${motorCenterX + 16} ${motorVisualCenterY - 4}`}
                  fill="none"
                  stroke={motorRingStroke}
                  strokeWidth="4.2"
                  strokeLinecap="round"
              ...
              ? phaseXs.map((x, index) => {
                  ...
                  return (
                    <g key={`flow-${index}`}>
                      ...
                      <PhaseFlow
                        path={loadPath}
                        active={motorRunning && !overloadTripped}
