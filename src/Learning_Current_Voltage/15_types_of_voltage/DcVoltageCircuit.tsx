...
import { SVG_HEIGHT, SVG_WIDTH } from "./logic";
import { GraphBase, GraphLabels } from "./ui";

export function DcCircuit({ voltage, current, isPlaying }: { voltage: number; current: number; isPlaying: boolean }) {
  const particleDuration = Math.max(0.8, 2.4 - current / 2);
  const glowOpacity = current < 0.5 ? 0.25 : current < 2 ? 0.55 : 0.9;

  const batteryX = 84;
  const batteryY = 76;
  const batteryWidth = 102;
  const batteryHeight = 110;
  ...
  return (
    <svg viewBox="0 0 520 220" className="h-56 w-full" role="img" aria-label="DC voltage circuit">
      <defs>
        <filter id="dcVoltageLedGlow" x="-80%" y="-80%" width="260%" height="260%">
      ...
      <text x="34" y="112" fontSize="16" fontWeight="700" fill="#15803d">DC</text>
      <text x="34" y="132" fontSize="16" fontWeight="700" fill="#15803d">Battery</text>
      ...
      <text x={batteryTerminalX + 18} y={batteryY + 68} fontSize="13" fontWeight="700" fill="#dc2626">
        DC Supply
      </text>
      ...
      {[
        ...
        <circle key={`dcv-node-${index}`} cx={point.x} cy={point.y} r="2.4" fill="#f8fafc" stroke="#94a3b8" strokeWidth="1.1" />
      ))}
      ...
      <svg
        ...
        overflow="visible"
        style={{ filter: current > 0.25 ? "url(#dcVoltageLedGlow)" : "none" }}
      >
      ...
      {[0, 1, 2].map((dot) => (
        <motion.circle
          ...
          transition={{ duration: particleDuration, repeat: Infinity, delay: dot * 0.35, ease: "linear" }}
        >
          {isPlaying && <animateMotion dur={`${particleDuration}s`} repeatCount="indefinite" begin={`${dot * 0.35}s`} path={flowPath} />}
        </motion.circle>
      ))}

      <text x="210" y="62" fontSize="12" fontWeight="700" fill="#dc2626">Voltage: {voltage.toFixed(1)}V</text>
      <text x="324" y="194" fontSize="12" fontWeight="700" fill="#2563eb">Current: {current.toFixed(2)}A</text>
    </svg>
  );
}

export function DcWaveform({ voltage, current }: { voltage: number; current: number }) {
  const voltageY = 95 - voltage * 3.2;
  const currentY = 95 - current * 16;

  return (
    <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="h-56 w-full" role="img" aria-label="DC waveform graph">
      <GraphBase title="DC Waveform" />
      <path d={`M70 ${voltageY} H470`} stroke="#dc2626" strokeWidth="4" fill="none" strokeLinecap="round" />
      <path d={`M70 ${currentY} H470`} stroke="#2563eb" strokeWidth="4" fill="none" strokeLinecap="round" />
      <GraphLabels />
    </svg>
  );
}
