  ...
  return (
    <div className="w-full text-slate-900">
      <div className="mx-auto max-w-7xl space-y-4">
        ...
        <section className={`rounded-3xl border p-4 shadow-sm md:p-5 ${mode === "short" ? "border-red-200 bg-red-50/80" : "border-green-200 bg-green-50/80"}`}>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className={`text-sm font-semibold ${mode === "short" ? "text-red-900" : "text-green-900"}`}>Core idea</p>
              <p className={`mt-1 text-sm leading-6 ${mode === "short" ? "text-red-800" : "text-green-800"}`}>
                {solved.explanation}
              </p>
            </div>
          ...
          <section className="space-y-4">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white/95 p-5 shadow-[0_18px_44px_rgba(15,23,42,0.08)] backdrop-blur md:p-6">
              <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-slate-500">
                <span className={`h-2 w-2 rounded-full ${mode === "short" ? "bg-red-500" : "bg-green-500"}`} />
                Risk View
              </div>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
                  ...
                  <p className="mt-2 text-[1.8rem] font-semibold text-blue-800">{flowLevel}</p>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-sm font-semibold text-slate-700">Power result</p>
                ...
                <div className={`rounded-2xl border p-4 ${mode === "short" ? "border-red-200 bg-red-50" : "border-green-200 bg-green-50"}`}>
                  <p className={`text-sm font-semibold ${mode === "short" ? "text-red-700" : "text-green-700"}`}>
                    Load status
                  </p>
                  <p className={`mt-2 text-[1.8rem] font-semibold ${mode === "short" ? "text-red-800" : "text-green-800"}`}>
                    {solved.loadActive ? "In Circuit" : "Bypassed"}
                  </p>
                  <p className={`mt-2 text-sm leading-6 ${mode === "short" ? "text-red-700/80" : "text-green-700/80"}`}>
                    {solved.loadActive
