    ...
    <div className="min-h-screen bg-slate-100 p-4 text-slate-900">
      <div className="mx-auto max-w-7xl rounded-3xl border border-slate-300 bg-white p-4 shadow-2xl">
        <header className="mb-4 flex items-center justify-between border-b border-slate-200 pb-4">
          ...
          <button
            onClick={reset}
            className="rounded-xl border border-blue-300 bg-blue-50 px-5 py-2 text-sm font-bold text-blue-700 hover:bg-blue-100"
          >
            Reset
          </button>
        </header>
        ...
        <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
          <aside className="space-y-4">
            ...
            <Panel title="Real-Time Readings">
              ...
              <Reading label="Power (P)" value={power.toFixed(2)} unit="W" color="text-blue-600" />
          ...
          <main className="space-y-4">
            <ElectricalCircuit current={current} angle={needleAngle} flowSpeed={flowSpeed} />
            <WaterSystem
        ...
        <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm font-bold text-blue-800">
          Industrial logic: Resistance up - pipe becomes narrower - water flow slows - current decreases.
        </div>
      </div>
    </div>
  );
