...
export function getFullWaveState(
  ...
  const waveform: WavePoint[] = Array.from({ length: samples }, (_, index) => {
    ...
    const activeDiode: ActiveDiode =
      Math.abs(vinTop) > profile.drop + ledForwardDrop
        ? vinTop >= 0
          ? "D1"
          : "D2"
        : "none";
    const vout = activeDiode === "none" ? 0 : rectifiedVin - profile.drop;
    const current =
      activeDiode === "none"
        ? 0
        : Math.max(0, (rectifiedVin - profile.drop - ledForwardDrop) / safeLoad);
